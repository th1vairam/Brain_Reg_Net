...
rosmap.key = downloadFile('syn3382527') %>%
  dplyr::select(projid, mrna_id) %>%
  tidyr::separate(mrna_id, c('a','b','c'), sep = '_') %>%
  tidyr::unite(SampleID, a,b, sep = '_') %>%
  dplyr::rename(Donor_ID = projid)
all.used.ids = c(all.used.ids, 'syn3382527')

covariates$ROSMAP = covariates$ROSMAP %>%
  left_join(rosmap.key) %>%
