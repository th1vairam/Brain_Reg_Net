...
covariates$ROSMAP = covariates$ROSMAP %>%
  dplyr::mutate(Gender = factor(msex, levels = c(0,1), labels = c('0' = 'FEMALE', '1' = 'MALE')),
                ...
                BrainRegion = 'DLPFC',
                Study = 'ROSMAP') %>%
  dplyr::select(SampleID, Study, BrainRegion, Diagnosis, Batch, Gender, RIN, AAD, PMI, PCT_PF_READS_ALIGNED,
...
write.table(logcpm, file = 'allReprocessedCounts.tsv', sep = '\t', row.names=F, quote=F)
