...
knit2synapse::knitToFolderEntity(file = "./MSSM_RNASeq_SVA_Reprocessed.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB Reprocessed RNASeq (with IRW SVA)')
```
...
source('../R/num.sv.R') # Source parallel version of num.sv from local repo
...
thisFileName <- 'MSSM_RNASeq_SVA_Reprocessed.Rmd'
...
COUNT <- COUNT[, ind]
```
`r paste('In total',length(indRemoved),'samples were removed')`
`r paste(indRemoved, collapse = ',')`
...
# RIN
p = list()
p[[1]] = ggplot(METADATA, aes(x = as.factor(Diagnosis), y = as.numeric(RIN), fill = as.factor(BrainRegion))) + geom_boxplot()
...
postAdjustCovars = c("BrainRegion.Diagnosis", "RIN", "Batch", "PMI", "AOD")
...
residualSigCovars = runPCAandPlotCorrelations(RESIDUAL.GENE_EXPRESSION, 
                                              COVARIATES, 'all adjusted design(voom-normalized)',
                                              isKeyPlot=TRUE)
```
### Sanity check
...
# Code
CODE <- Folder(name = "MSBB Reprocessed RNASeq (with IRW SVA)", parentId = parentId)
CODE <- synStore(CODE)
