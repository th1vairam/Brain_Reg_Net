...
knit2synapse::knitToFolderEntity(file = "./Mayo_RNASeq_Reprocessed_AdjustPMI.Rmd",
                                 parentId = "syn5570325",
                                 entityName = 'UFL_Mayo_ISB RNASeq Reprocessed Counts (with SVA based PMI adjustments)')
```
...
thisFileName <- 'Mayo_RNASeq_Reprocessed_AdjustPMI.Rmd'
...
cntrl = rep(0.5, dim(VOOM.ADJUSTED.GENE_EXPRESSION$E)[1])
names(cntrl) = sapply(rownames(VOOM.ADJUSTED.GENE_EXPRESSION$E), function(x){unlist(str_split(x,'\\.'))[1]})
cntrl[intersect(unlist(CONTROL.PROBES), names(cntrl))] = 1 

# Get actual and null model
...
SURR.VAR = sva(VOOM.ADJUSTED.GENE_EXPRESSION$E, 
               MODEL1, MODEL0, 
               n.sv = NUM.SV, 
               controls = cntrl,
               method = "supervised", 
...
```{r diffExp, fig.height=15, fig.width=25}
# Fit contrast
...
tmp = DE %>% 
  ...
  dplyr::mutate(D = logFC/abs(logFC)) %>%
  group_by(Comparison, D) %>%
  dplyr::summarize(nDiffExpGenes = length(unique(Gene.ID))) %>%
  spread(D, nDiffExpGenes)
kable(tmp)

DE = DE %>%
  left_join(backgroundGenes) %>% left_join(Ensemble2HGNC) %>%
  dplyr::select(Gene.ID, ensembl_gene_id, hgnc_symbol, Comparison, BrainRegion, Name, 
...
# Code
CODE <- Folder(name = 'UFL_Mayo_ISB RNASeq Reprocessed Counts (with SVA based PMI adjustments)', parentId = parentId)
CODE <- synStore(CODE)
...
SURR.VAR = rownameToFirstColumn(SURR.VAR, 'ID')
write.table(SURR.VAR, file = 'MAYOsurrogatevariable.tsv', sep = '\t', row.names=F, quote=F)
SURR_OBJ = File('MAYOsurrogatevariable.tsv', name = 'Surrogate Variables', parentId = CODE$properties$id)
SURR_OBJ = synStore(SURR_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                    executed = thisFile, activityDescription = activityDescription)
