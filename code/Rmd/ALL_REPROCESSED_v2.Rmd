...
knit2synapse::knitToFolderEntity(file = "./ALL_REPROCESSED_v2.Rmd",
                                 parentId = "syn8672415",
                                 entityName = 'ALL Reprocessed RNASeq (Normalised - sparse model)')
```
...
thisFileName <- 'ALL_REPROCESSED_v2.Rmd'
...
covariates$ROSMAP = covariates$ROSMAP %>%
  dplyr::mutate(SampleID = as.character(SampleID),
                ...
                msex = factor(msex, levels = c('1', '0'), labels = c('1'='MALE','0'='FEMALE')),
                Batch = factor(Batch),
                BrainRegion.ApoE = paste('DLPFC', apoe_genotype, sep = '.')) %>%
  ...
  dplyr::select(SampleID, BrainRegion.Diagnosis, Batch, Gender, AOD, PMI, RIN, PCT_PF_READS_ALIGNED, 
                PCT_CODING_BASES, PCT_INTERGENIC_BASES, PCT_INTRONIC_BASES, PCT_RIBOSOMAL_BASES, BrainRegion.ApoE)
...
covariates$MSBB = covariates$MSBB %>%
  dplyr::mutate(SampleID = as.character(SampleID),
                ...
                SEX = factor(SEX, levels = c('M', 'F'), labels = c('M'='MALE','F'='FEMALE')),
                batch = factor(batch),
                PMI = PMI/60) %>%
  ...
  dplyr::select(SampleID, BrainRegion.Diagnosis, individualIdentifier, Batch, Gender, AOD, PMI, RIN, 
                ...
                PCT_RIBOSOMAL_BASES, BrainRegion.ApoE)

covariates$MAYO = covariates$MAYO %>%
  dplyr::mutate(SampleID = as.character(SampleID),
                Donor_ID = factor(paste0('MAYO', Donor_ID)),
                Gender = factor(Gender, levels = c('M', 'F'), labels = c('M'='MALE','F'='FEMALE')),
                Batch = factor(Source)) %>%
  dplyr::rename(individualIdentifier = Donor_ID, AOD = AgeAtDeath) %>%
  dplyr::select(SampleID, BrainRegion.Diagnosis, individualIdentifier, Batch, Gender, AOD, PMI, RIN, 
                PCT_PF_READS_ALIGNED, PCT_CODING_BASES, PCT_INTERGENIC_BASES, PCT_INTRONIC_BASES, 
                PCT_RIBOSOMAL_BASES, BrainRegion.ApoE)
levels(covariates$MAYO$BrainRegion.Diagnosis) = c("CER.AD", "CER.CONTROL", "CER.OTHER", "TCX.AD", "TCX.CONTROL", "TCX.OTHER")
...
covariates = rbindlist(covariates, use.names = T, fill = T, idcol = 'Study') %>%
  ...
  dplyr::mutate(BrainRegion = factor(BrainRegion, levels = c('DLPFC', 'FP', 'IFG', 'PHG', 'STG', 'CER', 'TCX')),
                BrainRegion.Diagnosis = factor(paste(BrainRegion, Diagnosis, sep = '.')),
                BrainRegion.ApoE = factor(as.character(BrainRegion.ApoE)))
...
FactorCovariates <- c("individualIdentifier", "Study", "BrainRegion.Diagnosis", "BrainRegion", "Diagnosis",
                      "Gender", "Batch", "BrainRegion.ApoE")
ContCovariates <- c("AOD", "PMI", "RIN", "PCT_PF_READS_ALIGNED", "PCT_CODING_BASES", 
...
indToRemove = c('367_120502', '1950_TCX', '1950_CER', '1925_TCX', '1957_CER')
...
sdev = round((PC$sdev^2/sum(PC$sdev^2)) * 100, 0)

p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=BrainRegion, shape=Study, size=RIN))
p <- p + theme_bw() + theme(legend.position="top") + ylab(paste0('PC1(',sdev[2],'%)')) + xlab(paste0('PC1(',sdev[1],'%)'))
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=BrainRegion, shape=Study, size=RIN))
p <- p + theme_bw() + theme(legend.position="top") + xlab(paste0('PC1(',round(100*PC$sdev[1]^2/sum(PC$sdev^2)),'%)')) 
p <- p + ylab(paste0('PC2(',round(100*PC$sdev[2]^2/sum(PC$sdev^2)),'%)')) 
p
```
    ...
    allModelBIC = foreach(i = 1:length(covarsToSelect),
                          ...
                          .export = c('fitMixedEffectModel')) %dopar% {
                            ...
                            modelBIC = fitMixedEffectModel(c(adjustCovars, covarsToSelect[i]),
                                                           covars[colnames(cnt),'individualIdentifier'], 
                                                           lcpm, 
                                                           cnt, 
                                                           covars) %>%
...
baseModelBIC = fitMixedEffectModel(adjust.covars, 
                                   COVARIATES[colnames(NEW.COUNTS),'individualIdentifier'], 
...
# Iteratively identify covariates in phase I
phase3.covars = c("PCT_PF_READS_ALIGNED", "PCT_CODING_BASES", "PCT_INTERGENIC_BASES", 
...
# Get design matrix
DESIGN = getDesignMatrix(COVARIATES[, c("BrainRegion.Diagnosis", postAdjustCovars), drop = F], Intercept = F)
...
# Get design matrix
DESIGN = getDesignMatrix(COVARIATES[, c('BrainRegion.ApoE', setdiff(postAdjustCovars, 'BrainRegion.Diagnosis')), drop = F],
                         Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
cntr = c()
for (region in c('DLPFC', 'FP', 'STG', 'PHG', 'IFG', 'CER', 'TCX')){
  tmp = combn(paste0('BrainRegion.ApoE', region, '_', c('0', '1', '2')),2)
  cntr = c(cntr, apply(tmp, 2, paste, collapse = '-'))
}
...
# Get design matrix
DESIGN = getDesignMatrix(COVARIATES[, c('BrainRegion.Diagnosis.Gender', setdiff(postAdjustCovars, c('BrainRegion.Diagnosis', 'Gender'))), drop = F], Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
```{r diff.exp4, fig.height=7, fig.width=24}
# Get design matrix
DESIGN = getDesignMatrix(COVARIATES[, c('BrainRegion.Diagnosis', setdiff(postAdjustCovars, 'AOD')), drop = F], Intercept = F)
...
Genes that are differentially expressed at an FDR <= 0.05 and folde change of 1.2 are
```{r diff.exp5, fig.height=10, fig.width=15, eval = F}
# Cell type markers
CELL.FRAC = downloadFile('syn10163089')
all.used.ids = c(all.used.ids, 'syn10163089')

# Merge covariates to cell type fraction
NEW.COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID') %>%
  left_join(CELL.FRAC)

my.theme = theme(legend.position = 'top', axis.text.x = element_text(angle = 45, hjust = 1))
pl = list()
pl[[1]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = astrocytes, color = Diagnosis)) + geom_boxplot() + ggtitle('Astrocytes') + my.theme
pl[[2]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = endothelial, color = Diagnosis)) + geom_boxplot() + ggtitle('Endothelial') + my.theme
pl[[3]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = microglia, color = Diagnosis)) + geom_boxplot() + ggtitle('Microglia') + my.theme
pl[[4]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = neurons, color = Diagnosis)) + geom_boxplot() + ggtitle('Neurons') + my.theme
pl[[5]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = myeloid, color = Diagnosis)) + geom_boxplot() + ggtitle('Myeloid') + my.theme
pl[[6]] = ggplot(NEW.COVARIATES, aes(x = BrainRegion, y = oligodendrocytes, color = Diagnosis)) + geom_boxplot() + ggtitle('Oligodendrocytes') + my.theme
multiplot(plotlist = pl, cols = 3)

# Get design matrix
DESIGN = getDesignMatrix(NEW.COVARIATES[, c(adjust.covars, "astrocytes", "endothelial", "microglia", "myeloid", "neurons", "oligodendrocytes", "OPC"), drop = F], Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
cntr = c()
for (region in  c('DLPFC', 'FP', 'STG', 'PHG', 'IFG', 'CER', 'TCX')){
  cnt = c(paste0('BrainRegion.Diagnosis', region, '_AD-BrainRegion.Diagnosis', region, '_CONTROL'))
  cntr = c(cntr, cnt)
}
contrast = makeContrasts(contrasts=cntr,
                         levels = colnames(FIT$coefficients))
cnames = c()
for (region in  c('DLPFC', 'FP', 'STG', 'PHG', 'IFG', 'CER', 'TCX')){
  cnt = c(paste0(region, '_AD_CELLFRAC-', region, '_CONTROL_CELLFRAC'))
  cnames = c(cnames, cnt)
}
colnames(contrast) = cnames
...
all.diff.exp = c(all.diff.exp, list(Diagnosis.CellFraction = DE))
all.fit = c(all.fit, list(Diagnosis.CellFraction = FIT))
```

### Gene set enrichment analysis
Using Fisher's exact test for gene set enrichment analysis
```{r enrich.de, fig.height=16, fig.width=16}
# Perform gene set enrichment analysis using wilcox test from limma
diffexp.genes = all.diff.exp %>%
data.table::rbindlist(idcol='Model', use.names = T, fill = T) %>%
dplyr::mutate(Direction = sign(logFC),
Direction = factor(Direction, labels = c('-1' = 'DOWN', '1' = 'UP')))

geneSet.test = diffexp.genes %>%
plyr::dlply(.(Model, Comparison, Direction), .fun = function(x){
unique(x$hgnc_symbol[x$adj.P.Val <= 0.05])
})

statistics.test = diffexp.genes %>%
plyr::dlply(.(Model, Comparison), .fun = function(x){
tmp = dplyr::select(x, hgnc_symbol, adj.P.Val) %>% dplyr::group_by(hgnc_symbol) %>% dplyr::top_n(1,-adj.P.Val) %>% unique() %>% as.data.frame()
rownames(tmp) = tmp$hgnc_symbol
return(tmp)
})
names(statistics.test) = paste(names(statistics.test),'UP',sep = '.')
statistics.test1 = statistics.test
names(statistics.test1) = gsub('.UP','.DOWN', names(statistics.test1))
statistics.test = c(statistics.test, statistics.test1)
...
geneSet.ref = GeneSets[c('KEGG_2016', 'GO_Biological_Process_2015', 'Reactome_2016', 'WikiPathways_2016')] %>%
lapply(function(x){
lapply(x, function(y){
...
enrichment.results = mapply(function(gs.test, st.test, gs.ref){
plyr::ldply(gs.ref, .fun = function(y, gs.test, st.test){
tmp = CovariateAnalysis::fisherEnrichment(gs.test, y, st.test$hgnc_symbol)
tmp$wilcox.pval = limma::geneSetTest(which(st.test$hgnc_symbol %in% y), st.test[,'adj.P.Val'])
return(tmp)
}, gs.test, st.test, .parallel = T, .id = 'GeneSetName') %>%
dplyr::mutate(fdr = p.adjust(pval, method = 'fdr'),
wilcox.fdr = p.adjust(wilcox.pval, method = 'fdr'))
}, geneSet.test, statistics.test[names(geneSet.test)], 
MoreArgs = list(geneSet.ref),
SIMPLIFY = F) %>%
data.table::rbindlist(idcol = 'Comparison', use.names = T, fill = T)
```
...
NEW.COUNTS %>%
  rownameToFirstColumn('Gene.ID') %>%
  write.table(file = 'AMP_nCounts.tsv', sep = '\t', row.names=F, quote=F)
NCOUNT_OBJ = File('AMP_nCounts.tsv', name = 'Counts (estimated)', parentId = CODE$properties$id)
...
write.table(diffexp.genes, file = 'AMP_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
...
write.table(enrichment.results, file = 'AMP_EnrichmentResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = File('AMP_EnrichmentResults.tsv', 
                  name = 'GeneSet Enrichment Results', 
                  parentId = CODE$properties$id)
annotations(ENRICH_OBJ) = annotations(CODE)
annotations(ENRICH_OBJ)$dataSubType = 'enrichment'
ENRICH_OBJ = synStore(ENRICH_OBJ, activity = synGetActivity(COV_OBJ$properties$id))

# Store genesets
save(list = 'geneSet.test', file = 'AMP_DiffExpGeneSet.RData')
DEG_OBJ = File('AMP_DiffExpGeneSet.RData',
               name = 'Differential Expression Gene Set', 
               parentId = CODE$properties$id)
annotations(DEG_OBJ) = annotations(CODE)
annotations(DEG_OBJ)$dataSubType = 'diffExpGS'
DEG_OBJ = synStore(DEG_OBJ, activity = synGetActivity(COV_OBJ$properties$id))

# Store all model fits
save(list = 'all.fit', file = 'AMP_allModelFits.RData')
AFIT_OBJ = File('AMP_allModelFits.RData',
                name = 'All Model Fit', 
                parentId = CODE$properties$id)
annotations(AFIT_OBJ) = annotations(CODE)
annotations(AFIT_OBJ)$dataSubType = 'allModelFit'
AFIT_OBJ = synStore(AFIT_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
synChangeEntityACL(AFIT_OBJ@properties$id, new.racl)
...
|  *Results*                                  |  *SynapseID*                     |
  ...
  |  Enrichment Results                         |  `r ENRICH_OBJ$properties$id`    |
  |  Differentially Expressed Gene Sets         |  `r DEG_OBJ$properties$id`       |