---
title: "Differential expression analysis of reprocessed AMPAD RNASeq data (All 7 brain regions with CQN normlaisation and BIC based model selection with study regressed apriori)"
...
knit2synapse::knitToFolderEntity(file = "./ALL_REPROCESSED_v2.Rmd",
                                 parentId = "syn8672415",
                                 entityName = 'ALL Reprocessed RNASeq (Normalised) - study effects removed')
```
...
fitMixedEffectModel <- function(varsToFit, groupingVar, lcpm, voomWeights, covars){
...
selectCovarsToAdjust <- function(covarsToSelect, adjustCovars, covars, lcpm, voomWeight, baseModelBIC){
  ...
  while(length(covarsToSelect) != 0){
    allModelBIC = foreach(i = 1:length(covarsToSelect),
                          .combine = c,
                          .packages = pkgs,
                          .export = c('fitMixedEffectModel','voomWeight')) %dopar% {
...
```{r regress.study}
# Get design matrix
DESIGN.MAT = getDesignMatrix(COVARIATES[colnames(NEW.COUNTS), 'Study', drop=F], Intercept = T)
DESIGN.MAT = DESIGN.MAT$design[,linColumnFinder(DESIGN.MAT$design)$indepCols]
  
# Estimate voom weights
VOOM.WEIGHTS = limma::voom(NEW.COUNTS, design = DESIGN.MAT, plot=F)

# Fit a linear model
FIT = lmFit(VOOM.WEIGHTS)

# Calculate residuals
RESIDUAL.EXPR = residuals.MArrayLM(FIT, VOOM.WEIGHTS$E)

# Find principal components of expression to plot
PC <- prcomp(RESIDUAL.EXPR, scale.=T, center = T)
...
adjust.covars = c('BrainRegion.Diagnosis', 'RIN', 'RIN2', 'Gender', 'PMI', 'AOD', 'Batch', 'PCT_RIBOSOMAL_BASES', 
                  'PCT_INTRONIC_BASES', 'PCT_CODING_BASES', 'PCT_PF_READS_ALIGNED', 'PCT_INTERGENIC_BASES')
```
...
VOOM.GENE_EXPRESSION$E = RESIDUAL.EXPR
...
Genes that are differentially expressed at an FDR <= 0.05 and folde change of 1.2 are
```{r diff.exp4.1, fig.height=10, fig.width=15}
# Cell type markers
...
# Code
CODE <- Folder(name = "ALL Reprocessed RNASeq (Normalised) - study effects removed", parentId = parentId)
