...
indToRetain = intersect(METADATA$Sampleid, colnames(EXPR))
...
RESIDUAL.GENE_EXPRESSION = RESIDUAL.GENE_EXPRESSION + (FIT$coefficients[,varsToAddBackIn] %*% t(data.matrix(DESIGN[,varsToAddBackIn])))
...
# Code
CODE <- Folder(name = "Differential Expression Analysis ROSMAP RNASeq (Based on cogdx)", parentId = parentId)
CODE <- synStore(CODE)
...
EXP_OBJ = synStore(EXPRP_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                   executed = thisFile, activityDescription = activityDescription)
```
|  *Results*            |  *SynapseID*                    |
|  -------              |   ---------                     |
