---
title: "Covariates analysis of MSBB RNASeq data of region BM 44 (realigned from source FASTQ with STAR-HTSeq)"
...
knit2synapse::knitToFolderEntity(file = "./MSSM_BM_44_RNASeq_Realigned_StarHTSeq.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB BM 44 RNASeq RealignedFromSource StarHTSeq')
```
...
thisFileName <- 'MSSM_BM_44_RNASeq_Realigned_StarHTSeq.Rmd'
...
METADATA = left_join(METADATA_TECHNICAL %>%
                       dplyr::mutate(ID = gsub('hB_RNA_','',NewBarcode),
                                     ID = as.numeric(ID)),
                     METADATA_CLINICAL %>%
                       dplyr::select(BB, BM.44.Barcode, PMI, RACE, 
...
# Code
CODE <- Folder(name = "MSBB BM 44 RNASeq  RealignedFromSource StarHTSeq", parentId = parentId)
CODE <- synStore(CODE)
