---
title: "Covariate analysis of UFL-Mayo-ISB RNASeq temporal cortex"
...
knit2synapse::knitToFolderEntity(file = "./Mayo_TemporalCortex_RNASeq.Rmd",
                                 parentId ='syn5612983',#"syn5570325",
                                 entityName = 'UFL_Mayo_ISB Temporal Cortex RNASeq')
```
...
activityDescription = 'Covariate analysis of UFL_Mayo_ISB temporal cortex RNASeq data';

thisFileName <- 'Mayo_TemporalCortex_RNASeq.Rmd'
...
METADATA <- METADATA %>%
  dplyr::filter(!(ID %in% c("1923_TCX")))
```
Following five samples were considered outliers and removed 1923\_TCX
```{r preprocess.data1}
...
FactorCovariates <- c("Source", "Diagnosis",  "FLOWCELL")
...
```{r decompse.normalise.data, fig.height=9, fig.width=16, results='asis'}
...
```
Adding "Diagnosis" back to the residuals
```{r addVarsBack}
...
```{r decompse.normalise.data1, fig.height=10, fig.width=16, results='asis'}
# Find principal components of expression to plot
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=factor(clust), shape=as.factor(Source), size=as.numeric(AgeAtDeath)))
p <- p + theme_bw() + theme(legend.position="top") 
...
plotDendroAndColors(sampleTrees, 
                    colors = labels2colors(cbind(data.frame(clust = clust), COVARIATES)), 
                    abHeight = 0.80, 
...
writeLines(paste('Samples that deviate from others are', paste(sampleTrees$labels[clust==0], collapse = ',')))
```
...
# Code
CODE <- Folder(name = 'UFL_Mayo_ISB Temporal Cortex RNASeq', parentId = parentId)
CODE <- synStore(CODE)
