...
knit2synapse::knitToFolderEntity(file = "./CMC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_ModSel.Rmd",
                                 parentId ="syn7501880",
                                 entityName = 'ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN (Model Selection)')
```
...
```
### Filter genes
Remove genes that have less than 5 cpm counts in at least 100% of samples per Dx.Tissue. Also remove genes with missing gene length and percentage GC content
```{r cpmnormalisation}
genesToAnalyze = dlply(METADATA, .(Dx.Tissue), .fun = function(mtd, count){
  processed.counts = getGeneFilteredGeneExprMatrix(count[,mtd$SampleID %>% droplevels()],
                                                   MIN_GENE_CPM=5, 
                                                   MIN_SAMPLE_PERCENT_WITH_MIN_GENE_CPM=1)
...
selectCovarsToAdjust <- function(covarsToSelect, adjustCovars, covars, lcpm, cnt, baseModelBIC){
  ...
  while(length(covarsToSelect) != 0){
    ...
    # Select variable
    tmp = nChange %>%
      dplyr::filter(nchange > round(dim(cnt)[1]*0.05)) %>%
...
adjust.covars = c('Dx.Tissue', 'Institution', 'Gender')
...
phase1.covars = c('PMI', 'Age_of_Death', 'RIN', 'RIN2', 'EV.1', 'EV.2', 'EV.3', 'EV.4', 'EV.5')
...
# Code
CODE <- Folder(name = "ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN (Model Selection)", parentId = parentId)
