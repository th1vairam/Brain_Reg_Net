...
knit2synapse::knitToFolderEntity(file = "./MSSM_RNASeq_cdr.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB RNASeq (based on CDR)')
```
...
thisFileName <- 'MSSM_RNASeq_cdr.Rmd'
...
postAdjustCovars = 'BrainRegion.CDR'
...
for (i in 1:dim(contrast)[2]){
  ...
  p[[i]] <- p[[i]] + ggtitle(gsub('BrainRegion.CDR','',colnames(contrast)[i])) + xlim(c(-1.5,1.5)) + ylim(c(0,6))
...
# Code
CODE <- Folder(name = "MSBB RNASeq (based on CDR)", parentId = parentId)
CODE <- synStore(CODE)
