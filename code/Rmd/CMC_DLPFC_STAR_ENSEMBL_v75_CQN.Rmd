---
title: "Covariate analysis of CMC RNASeq data reprocessed with STAR-feature counts workflow with Ensembl v75 (DLPFC with CQN normlaisation)"
...
knit2synapse::knitToFolderEntity(file = "./CMC_DLPFC_STAR_ENSEMBL_v75_CQN.Rmd",
                                 parentId = "syn7501880",
                                 entityName = 'DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN')
```
...
thisFileName <- 'CMC_DLPFC_STAR_ENSEMBL_v75_CQN.Rmd'
...
METADATA = METADATA_DLPFC %>%
  droplevels() %>%
  list(.,METADATA_QC_DLPFC) %>%
  join_all %>%
...
# Based on PC
indToRemove = c('MSSM_RNA_PFC_133', 'MSSM_RNA_PFC_139', 'MSSM_RNA_PFC_163', 'MSSM_RNA_PFC_260',
                'MSSM_RNA_PFC_299', 'MSSM_RNA_PFC_361', 'MSSM_RNA_PFC_89', 'MSSM_RNA_PFC_319', 'MSSM_RNA_PFC_348',
                'MSSM_RNA_PFC_354', 'MSSM_RNA_PFC_60', 'PENN_RNA_PFC_71', 'PENN_RNA_PFC_56',  "MSSM_RNA_PFC_230",
                "MSSM_RNA_PFC_304", "MSSM_RNA_PFC_309", "MSSM_RNA_PFC_329", "MSSM_RNA_PFC_82",
...
# Code
CODE <- Folder(name = "DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN", parentId = parentId)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_DLPFC_Counts.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_DLPFC_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$y %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_DLPFC_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = File('CMC_DLPFC_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_DLPFC_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = File('CMC_DLPFC_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
...
DESIGN %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'CMC_DLPFC_Design.tsv', sep = '\t', row.names=F, quote=F)
...
RESIDUAL.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_DLPFC_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_DLPFC_netResidualExpression.tsv', 
...
write.table(all.diff.exp, file = 'CMC_DLPFC_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_DLPFC_DiffExpression.tsv', 
...
write.table(enrichment.results, file = 'CMC_DLPFC_EnrichResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = File('CMC_DLPFC_EnrichResults.tsv', 
