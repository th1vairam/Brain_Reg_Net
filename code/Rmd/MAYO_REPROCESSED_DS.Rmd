---
title: "Covariate analysis of MAYO 50% downsampled reprocessed counts (with CQN normalisation and covariates adjusted)"
...
knit2synapse::knitToFolderEntity(file = "./MAYO_REPROCESSED_DS.Rmd",
                                 parentId = "syn8672415",
                                 entityName = 'MAYO 50percent Down Sampled Reprocessed RNASeq')
```
...
thisFileName <- 'MAYO_REPROCESSED_DS.Rmd'
...
```{r download.data}
# Get reprocessed sample counts for cerebellum and temporal cortex
COUNT_ID = 'syn11635112'
...
METADATA_TC <- fread(synGet(METADATA_TC_ID)@filePath,fill=T) %>%
...
METADATA_CE <- fread(synGet(METADATA_CE_ID)@filePath, fill = T) %>%
...
# Fit linear model using new weights dplyr::count()and new design
VOOM.WEIGHTS$E = CQN.GENE_EXPRESSION$E
FIT = lmFit(VOOM.WEIGHTS$E,
            weights = VOOM.WEIGHTS$weights,
            design = VOOM.WEIGHTS$design,
...
tmp = DE %>%
  dplyr::select(ensembl_gene_id, Comparison, Direction, BrainRegion.ref, AOD.ref) %>%
...
all.annotations = list(
  ...
  summaryLevel = 'gene',
  assay	 = 'RNAseq',
  assaySubType = 'Reprocessed down sampled',
...
# Code
CODE <- Folder(name = "MAYO 50percent Down Sampled Reprocessed RNASeq", parentId = parentId)
