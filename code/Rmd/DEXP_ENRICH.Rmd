...
knit2synapse::knitToFolderEntity(file = "./DEXP_ENRICH.Rmd",
                                 parentId = "syn8672415",
...
dexp$Direction = 'NONE'
dexp$Direction[dexp$adj.P.Val <= 0.05 & dexp$logFC >= log2(1.2)] = 'UP'
dexp$Direction[dexp$adj.P.Val <= 0.05 & dexp$logFC <= -log2(1.2)] = 'DOWN'
dexp$Tissue[dexp$Tissue == 'CER'] = 'CBE'
...
Ensemble2HGNC <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"),
                       ...
                       values = unique(dexp$ensembl_gene_id),
                       mart = mart)
dexp = dexp %>%
...
all.used.ids = c('syn4867851')
GeneSets = GeneSets[c('BioCarta', 'Chromosome_Location', 'Cross_Species_Phenotype', 
                      'Disease_Signatures_from_GEO_down', 'Disease_Signatures_from_GEO_up',
                      'ENCODE_TF_ChIP-seq', 'GO_Biological_Process', 
                      'KEGG_2015', 'Mouse_Gene_Atlas', 'OMIM_Disease', 'OMIM_Expanded', 'Panther', 
                      'Reactome', 'TargetScan_microRNA', 'TF-LOF_Expression_from_GEO', 'TRANSFAC_and_JASPAR_PWMs',
                      'WikiPathways', 'Single_Gene_Perturbations_from_GEO_up',
                      'Single_Gene_Perturbations_from_GEO_down', 'Aging_Perturbations_from_GEO_down',
                      'Aging_Perturbations_from_GEO_up', 'dbGaP', 'Disease_Perturbations_from_GEO_down',
                      'Disease_Perturbations_from_GEO_up', 'DrugMatrix', 
                      'Drug_Perturbations_from_GEO_down', 'Drug_Perturbations_from_GEO_up', 
                      'GTEx_Tissue_Sample_Gene_Expression_Profiles_down',
                      'GTEx_Tissue_Sample_Gene_Expression_Profiles_up', 'Jensen_DISEASES', 
...
enrich.results = dexp %>%
  ...
  plyr::ddply(.(Model, Tissue, Comparison, Gender, Direction), .fun = function(x, gs, bckGenes){
    ...
    enrich = plyr::ldply(gs, .fun = function(y, gsToTest, bckGenes){
      ...
      }, gsToTest, bckGenes, .id = 'SetName', .parallel = T, .paropts = list(.packages = c('CovariateAnalysis')))
