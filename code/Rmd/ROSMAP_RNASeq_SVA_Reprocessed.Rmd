---
title: "Covariate analysis of ROSMAP Reprocessed RNASeq (with SVA)"
...
knit2synapse::knitToFolderEntity(file = "./ROSMAP_RNASeq_SVA_Reprocessed.Rmd",
                                 parentId ="syn5570291",
                                 entityName = 'ROSMAP RNASeq Reprocessed Counts (Based on cogdx with SVA IRW)')
```
...
thisFileName <- 'ROSMAP_RNASeq_SVA_Reprocessed.Rmd'
...
#          .export = c('PROCESSED_COUNTS','MODEL1')) %dopar% {
#   PERM.COUNTS <- t(apply(PROCESSED_COUNTS$filteredExprMatrix, 1, sample, replace = FALSE))
...
# Estimate voom weights
VOOM.ADJUSTED.GENE_EXPRESSION = voom(PROCESSED_COUNTS$filteredExprMatrix, design=cbind(MODEL1, SURR.VAR), plot=F)
...
row.id = data.frame(GeneID = rownames(RESIDUAL.GENE_EXPRESSION),
                    ensembl_gene_id = rownames(RESIDUAL.GENE_EXPRESSION)) %>%
  separate(ensembl_gene_id, c('ensembl_gene_id','position'), sep = '\\.')
...
Ensemble2HGNC <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"),
                       filters = "ensembl_gene_id", values = row.id$ensembl_gene_id,
                       mart = mart)
...
p = list()
for (i in 1:3){
  DE[[i]] = topTable(FIT.CONTR, coef=i, number = dim(COUNT)[1])
...
p = p + geom_vline(xintercept = -log2(1.2), color = 'red') + facet_grid(.~Comparison, scales = 'fixed')
p
```
### Store files in synapse
```{r synapse.store, include=FALSE, eval=TRUE, cache=FALSE}
# Code
CODE <- Folder(name = "ROSMAP RNASeq Reprocessed Counts (Based on cogdx with SVA IRW)", parentId = parentId)
CODE <- synStore(CODE)
...
# Store SVs
write.table(SURR.VAR, file = 'ROSMAPsurrogatevar.tsv', sep = '\t', row.names=F, quote=F)
SURR_OBJ = File('ROSMAPsurrogatevar.tsv', name = 'Surrogate Variables', parentId = CODE$properties$id)
...
|  Surrogate variable       |  `r SURR_OBJ$properties$id`      |
