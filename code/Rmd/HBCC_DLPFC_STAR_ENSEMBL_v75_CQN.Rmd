---
title: Covariate analysis of HBCC RNASeq data reprocessed with STAR-feature counts workflow with Ensembl v75 (ACC and DLPFC with CQN normlaisation)
...
knit2synapse::knitToFolderEntity(file = "./HBCC_DLPFC_STAR_ENSEMBL_v75_CQN.Rmd",
                                 parentId = "syn9872607", #"syn7501880", 
...
# Estimate voom weights
VOOM.GENE_EXPRESSION1 = limma::voom(cnts, design = DESIGN1)
...
### Differential expression analysis (DLPFC, SCZ - Control, Female - Male)
Genes that are differentially expressed at an FDR <= 0.05 are
...
# Estimate voom weights
VOOM.GENE_EXPRESSION2 = voom(cnts, design = DESIGN2)
...
RESIDUAL.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'HBCC_DLPFC_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('HBCC_DLPFC_netResidualExpression.tsv', 
...
write.table(DE, file = 'HBCC_DLPFC_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('HBCC_DLPFC_DiffExpression.tsv', 
...
write.table(enrichment.results, file = 'HBCC_DLPFC_EnrichResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = File('HBCC_DLPFC_EnrichResults.tsv', 
