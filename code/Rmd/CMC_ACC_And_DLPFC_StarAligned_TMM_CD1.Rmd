---
title: "Covariate analysis of CMC RNASeq data reprocessed with Star-Feature counts pipeline (ACC and DLPFC with TMM normlaisation and Custom Design 1)"
...
knit2synapse::knitToFolderEntity(file = "./CMC_ACC_And_DLPFC_StarAligned_TMM_CD1.Rmd",
                                 parentId ="syn7501880",
                                 entityName = 'ACC and DLPFC - STAR Reprocessed - with TMM - Custom Design 1')
```
...
thisFileName <- 'CMC_ACC_And_DLPFC_StarAligned_TMM_CD1.Rmd'
...
```{r iterativeNorm, results='asis'}
# Primary variable of interest
postAdjustCovars = c('Dx.Tissue', 'Institution', 'Gender', 'IntronicRate', 'IntergenicRate', 'MappedReads', 'PercentAligned',
                     'RIN', 'PMI', 'Age_of_Death', 'rRNARate', 'RibozeroBatch', 'EV.1', 'EV.2', 'EV.3', 'EV.4', 'EV.5',
                     'GenesDetected');
...
tmp = DE %>%
  dplyr::mutate(dir = logFC/abs(logFC)) %>%
  dplyr::filter(adj.P.Val <= 0.05) %>%
  dplyr::select(ensembl_gene_id, Comparison, Tissue, dir) %>%
  group_by(Comparison, Tissue, dir) %>%
...
```{r gc.gx, fig.width = 7, fig.height = 14}
tmp = left_join(DE, GC.CONT) %>%
  left_join(Gx.LEN) %>%
  unique

p = list()
p[[1]] = ggplot(tmp, aes(x = log10(transcript_length), y = logFC)) + geom_point() + geom_smooth(method = 'lm')
p[[1]] = p[[1]] + xlab('Log10 of Maximum Tx Length')

p[[2]] = ggplot(tmp, aes(x = log10(percentage_gc_content), y = logFC)) + geom_point() + geom_smooth(method = 'lm')
p[[2]] = p[[2]] + xlab('Percent GC Content')
CovariateAnalysis::multiplot(plotlist = p, cols = 2)
```

### Store files in synapse
```{r synapse.store, include=FALSE, eval=TRUE, cache=FALSE}
# Code
CODE <- Folder(name = "ACC and DLPFC - STAR Reprocessed - with TMM - Custom Design 1", parentId = parentId)
CODE <- synStore(CODE)
...
# Store design matrix
write.table(DM1$design, file = 'CMC_ACC_Design.tsv', sep = '\t', row.names=T, quote=F)
