...
knit2synapse::knitToFolderEntity(file = "./MSSM_RNASeq_PlaqueMean.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB RNASeq (with adjustments for mean plaque)')
```
...
thisFileName <- 'MSSM_RNASeq_PlaqueMean.Rmd'
...
ContCovariates <- c('PMI', 'AOD', 'RIN', 'RIN2', 'PlaqueMean')
...
preAdjustedSigCovars = runPCAandPlotCorrelations(VOOM.GENE_EXPRESSION$E, COVARIATES,'NULL design(voom-normalized)', isKeyPlot=TRUE, MIN_PVE_PCT_PC = 1)
```
...
VOOM.ADJUSTED.GENE_EXPRESSION$E <- left_join(rownameToFirstColumn(VOOM.ADJUSTED.GENE_EXPRESSION$E, "ensembl_gene_id"),
                                             Ensemble2HGNC) %>%
  dplyr::select(ensembl_gene_id, hgnc_symbol, one_of(rownames(COVARIATES)))
```
...
# Code
CODE <- Folder(name = "MSBB RNASeq (with adjustments for mean plaque)", parentId = parentId)
CODE <- synStore(CODE)
