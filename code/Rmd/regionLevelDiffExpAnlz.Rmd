...
```{r fxn}
performDifferentialExpression <- function(cov, cnt, contVar, varOfInterest, n){
  
  diffExp = foreach(i = 1:n, 
                    .combine = rbind,
                    .packages = c('limma','edgeR','cqn','dplyr','plyr','CovariateAnalysis')) %dopar% {
...
```{r diff.exp}
all.results = plyr::dlply(covariates, .(BrainRegion), .fun = function(cov, cnt){
  n = 5
  ...
  baseDiffExp <- performDifferentialExpression(cov, cnt, contVar, varOfInterest, n)
  ...
  genderDiffExp <- performDifferentialExpression(cov, cnt, contVar, varOfInterest, n)
