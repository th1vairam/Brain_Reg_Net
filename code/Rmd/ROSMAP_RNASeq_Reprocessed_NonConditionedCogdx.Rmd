...
knit2synapse::knitToFolderEntity(file = "./ROSMAP_RNASeq_Reprocessed_NonConditionedCogdx.Rmd",
                                 parentId ="syn5570291",
                                 entityName = 'ROSMAP RNASeq Reprocessed Counts UnConditioned (Based on cogdx)')
```
...
thisFileName <- 'ROSMAP_RNASeq_Reprocessed_NonConditionedCogdx.Rmd'
...
# Post adjusted design matrix
DM1 = getDesignMatrix(COVARIATES[,'cogdx',drop=F],Intercept = F)
DM1$design = DM1$design[,linColumnFinder(DM1$design)$indepCols, drop=F]
  
# Fit linear model using residuals and new design matrix
FIT = lmFit(RESIDUAL.GENE_EXPRESSION, DM1$design)
...
# Code
CODE <- Folder(name = "ROSMAP RNASeq Reprocessed Counts UnConditioned (Based on cogdx)", parentId = parentId)
CODE <- synStore(CODE)
