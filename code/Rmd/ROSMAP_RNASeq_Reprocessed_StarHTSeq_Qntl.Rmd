---
title: "Covariate analysis of ROSMAP Reprocessed RNASeq data aligned using STAR and quantified using HTSeq with quantile normalisation"
...
knit2synapse::knitToFolderEntity(file = "./ROSMAP_RNASeq_Reprocessed_StarHTSeq_Qntl.Rmd",
                                 parentId ="syn5570291",
                                 entityName = 'ROSMAP RNASeq Reprocessed Counts Star-HTSeq Qntl (Based on cogdx)')
```
...
thisFileName <- 'ROSMAP_RNASeq_Reprocessed_StarHTSeq_Qntl.Rmd'
...
VOOM.GENE_EXPRESSION = voom(PROCESSED_COUNTS$filteredExprMatrix, design=NULL, plot=T, normalize.method = "quantile")
```
...
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 100){
  ...
  VOOM.ADJUSTED.GENE_EXPRESSION = voom(PROCESSED_COUNTS$filteredExprMatrix, design=DM1$design, plot=F, normalize.method = "quantile")
...
# Code
CODE <- Folder(name = "ROSMAP RNASeq Reprocessed Counts Star-HTSeq Qntl (Based on cogdx)", parentId = parentId)
CODE <- synStore(CODE)
