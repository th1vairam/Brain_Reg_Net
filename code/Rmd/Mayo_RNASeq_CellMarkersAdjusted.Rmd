---
title: "Covariate analysis of UFL-Mayo-ISB RNASeq (both cerebellum and temporal cortex)"
...
knit2synapse::knitToFolderEntity(file = "./Mayo_RNASeq_CellMarkersAdjusted.Rmd",
                                 parentId = "syn5570325",
                                 entityName = 'UFL_Mayo_ISB RNASeq Cell Markers Adjusted')
```
...
thisFileName <- 'Mayo_RNASeq_CellMarkersAdjusted.Rmd'
...
### Cell type markers (i.e., eigen genes of cell types) as additional covariates
```{r cellMark, fig.height=10, fig.width = 15}
load(synGet('syn4893059')@filePath)
GeneSets = GeneSets[grep("Zhang", names(GeneSets))] %>%
  ldply(.fun = function(x){
    data.frame(hgnc_symbol = x)
  }, .id = 'CellType') %>%
  dplyr::mutate(CellType = gsub("Zhang:","",CellType)) %>%
  left_join(Ensemble2HGNC) %>%
  filter(!is.na(ensembl_gene_id))

kable(data.frame(table(GeneSets$CellType)) %>%
        dplyr::rename(CellType = Var1, NumberOfGenes = Freq))

COVARIATES.cellmarkers = dlply(GeneSets, .(CellType), .fun = function(gs){
  EXPR = COUNT[rownames(COUNT) %in% gs$ensembl_gene_id, ]
  PC <- prcomp(EXPR, scale.=T, center = T)
  rownameToFirstColumn(PC$rotation[,1, drop=F], 'ID') %>%
    plyr::rename(c('PC1' = unique(gs$CellType)))
}) %>%
  plyr::join_all(by = 'ID') 

METADATA = join_all(list(METADATA, COVARIATES.cellmarkers))

p = METADATA %>%
  dplyr::select(Diagnosis, Tissue, one_of(colnames(COVARIATES.cellmarkers))) %>%
  gather(CellType, EigenGeneExpression, -ID, -Diagnosis, -Tissue) %>%
  ggplot(aes(x = Diagnosis, y = abs(EigenGeneExpression), fill = Tissue)) + geom_boxplot() + facet_grid(.~CellType)
...
ContCovariates <- c("RIN", "RIN2", "AgeAtDeath", "Astrocyte", "Endothelial", "Microglia", "MyelinOligos", "Neuron", "NewOligos", "OPC")
...
```{r decompse.normalise.data1, fig.height=15, fig.width=8, results='asis'}
...
p <- p + geom_point(aes(shape=factor(Source), color=as.factor(Tissue), size=as.numeric(OPC)))
p <- p + theme_bw() + theme(legend.position="top") +scale_shape_manual(values = 1:2)
# p <- p + geom_text(aes(label= ID), size=4, hjust=0)
pl[[1]] <- p + xlab(paste0('PC1(', round(PC$sdev[1]^2/sum(PC$sdev^2)*100), '%)')) + ylab(paste0('PC2(', round(PC$sdev[2]^2/sum(PC$sdev^2)*100), '%)'))

p <- ggplot(plotdata, aes(x=PC3, y=PC4))
p <- p + geom_point(aes(shape=factor(Source), color=as.factor(Diagnosis), size=as.numeric(OPC)))
p <- p + theme_bw() + theme(legend.position="top") +scale_shape_manual(values = 1:2)
# p <- p + geom_text(aes(label= ID), size=4, hjust=0)
pl[[2]] <- p + xlab(paste0('PC3(', round(PC$sdev[3]^2/sum(PC$sdev^2)*100), '%)')) + ylab(paste0('PC4(', round(PC$sdev[4]^2/sum(PC$sdev^2)*100), '%)'))
...
WGCNA::plotDendroAndColors(sampleTrees, 
                           labels2colors(cbind(tree,COVARIATES.tmp[, FactorCovariates])),
                           groupLabels = c('Cluster',colnames(COVARIATES.tmp[, FactorCovariates])))
dev.off()
...
```{r decompse.normalise.data3, fig.height=15, fig.width=8, results='asis'}
# Find principal components of expression to plot
...
plotDendroAndColors(sampleTrees, 
                    colors = labels2colors(cbind(data.frame(clust = clust), COVARIATES.tmp[,FactorCovariates])), 
                    abHeight = 0.80, 
...
for (i in 1:12){
  ...
  p[[i]] <- p[[i]] + geom_vline(xintercept = -log2(1.5), color = 'red')
  p[[i]] <- p[[i]] + geom_vline(xintercept = log2(1.5), color = 'red')
...
tmp = DE %>% 
  dplyr::filter(adj.P.Val <= 0.05, abs(logFC) >= log2(1.5)) %>%
  group_by(Comparison) %>%
...
# Code
CODE <- Folder(name = 'UFL_Mayo_ISB RNASeq Cell Markers Adjusted', parentId = parentId)
CODE <- synStore(CODE)
