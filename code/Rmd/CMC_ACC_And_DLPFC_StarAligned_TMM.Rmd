---
title: "Covariate analysis of CMC RNASeq data reprocessed with Star-Feature counts pipeline (ACC and DLPFC with TMM normlaisation)"
...
Since many covariates are correlated, re-normalising COUNTS with an iterative design matrix. Here "Dx.Tissue" is chosen as the primary variable of interest (i.e., covariate selection is conditioned on diagnosis and tissue)

Adding Instituion and Gender a priori to variable selection as they are correlated with Dx.Tissue
...
genesets.to.test = sapply(geneset.files$file.id, function(id){
  tmp = downloadFile(id) %>%
...
enrich.Results = llply(diffexp.genes, .fun = function(x, genesetsToTest, backgroundGenes){
...
# Code
CODE <- Folder(name = "ACC and DLPFC - STAR Reprocessed - with TMM", parentId = parentId)
CODE <- synStore(CODE)
...
# Store results of enrichment analysis
write.table(enrich.Results, file = 'CMC_DLPFC_ACC_Enrichment.tsv', sep = '\t', row.names=F, quote=F)
ENR_OBJ = File('CMC_DLPFC_ACC_Enrichment.tsv', name = 'Enrichment Analysis', parentId = CODE$properties$id)
ENR_OBJ = synStore(ENR_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                    executed = thisFile, activityDescription = activityDescription)
stopCluster(cl)
```
|  *Results*                |  *SynapseID*                     |
