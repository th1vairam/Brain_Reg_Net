...
```{r dataExplore, fig.width = 6, fig.height = 14}
...
```
### CPM Normalisation
Preprocess count matrix and metadata. Determine design matrix for normalisation and differential expression analysis. 
...
Since many covariates are correlated, re-normalising COUNTS with an iterative design matrix. Here "Dx" is chosen as the primary variable of interest (i.e., covariate selection is conditioned on diagnosis)

Adding Instituion and Gender apriori to variable selection as they are correlated with Dx
Also adding first 5 eigen vectors computed from ancestry
```{r iterativeNorm, results='asis'}
# Primary variable of interest
postAdjustCovars = c('Dx', 'Institution', 'Gender');
