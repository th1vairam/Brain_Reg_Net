---
title: "Covariates analysis of MSBB RNASeq data of region BM 36 (realigned from synapse BAM with Picard-STAR-HTSeq)"
...
knit2synapse::knitToFolderEntity(file = "./MSSM_BM_36_RNASeq_Reprocessed_SynBAM_StarHTSeq.Rmd",
                                 parentId ="syn5570248",
                                 entityName = "MSBB BM 36 RNASeq RealignedFromSynBAM PicardStarHTSeq")
```
...
thisFileName <- 'MSSM_BM_36_RNASeq_Reprocessed_SynBAM_StarHTSeq.Rmd'
...
# Get sample ids
ind.reseq = grep('resequenced', colnames(COUNT))
ind.all = setdiff(1:dim(COUNT)[2], ind.reseq)

SampleID = rbind(data.frame(SampleID = colnames(COUNT)[ind.all],
                      ID = colnames(COUNT)[ind.all]) %>% 
  tidyr::separate(ID, c('A','B','ID', 'C'), sep = '_') %>%
  dplyr::mutate(ID = gsub('.accepted','',ID)) %>%
  dplyr::select(SampleID, ID) %>%
  dplyr::mutate(ID = as.numeric(ID),
                Sample.ID = SampleID,
                SampleID = paste0('BM_36_',ID)),
  data.frame(SampleID = colnames(COUNT)[ind.reseq],
             ID = colnames(COUNT)[ind.reseq]) %>% 
  tidyr::separate(ID, c('A','B','ID', 'C'), sep = '_') %>%
  dplyr::mutate(ID = gsub('.accepted','',ID)) %>%
  dplyr::select(SampleID, ID) %>%
  dplyr::mutate(ID = as.numeric(ID),
                Sample.ID = SampleID,
                SampleID = paste0('BM_36_Reseq_',ID)))
...
rownames(SampleID) = SampleID$Sample.ID
colnames(COUNT) = SampleID[colnames(COUNT), 'SampleID']
...
METADATA_TECHNICAL <- read.table(synGet(METADATA_TECHNICAL_ID)@filePath,sep='\t',header=T, row.names=1) %>%
...
# Code
CODE <- Folder(name = "MSBB BM 36 RNASeq RealignedFromSynBAM PicardStarHTSeq", parentId = parentId)
CODE <- synStore(CODE)
...
# Store logcpm values
write.table(VOOM.ADJUSTED.GENE_EXPRESSION$E, file = 'logCPM.tsv', sep = '\t', row.names=T, quote=F)
exp1_obj = File('logCPM.tsv', name = 'Raw logCPM', parentId = CODE$properties$id)
exp1_obj = synStore(exp1_obj, used = ALL_USED_IDs, activityName = activityName, 
                   executed = thisFile, activityDescription = activityDescription)
```
|  *Results*                |  *SynapseID*                    |
|  -------                  |   ---------                     |
