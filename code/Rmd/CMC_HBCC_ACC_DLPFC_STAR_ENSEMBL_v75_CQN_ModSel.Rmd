...
knit2synapse::knitToFolderEntity(file = "./CMC_HBCC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_ModSel.Rmd",
                                 parentId = "syn7501880",
                                 entityName = 'CMC and HBCC - ACC - DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN')
```
...
thisFileName <- 'CMC_HBCC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_ModSel.Rmd'
...
COVARIATES = COVARIATES %>%
  dplyr::filter(!(Dx.Tissue %in% c('BP_DLPFC', 'Other_ACC', 'Other_DLPFC')))
...
FactorCovariates <- c('Individual_ID', 'Institution', 'Gender', 'LibraryBatch', 'Dx.Tissue.Study',
                      'RibozeroBatch', 'FlowcellBatch')
ContCovariates <- c("Age_of_Death", "PMI", "RIN", "MappedReads", "IntragenicRate", "IntronicRate",
                    "IntergenicRate", "GenesDetected", "TranscriptsDetected", "ExpProfEfficiency", "rRNARate")
...
genesToAnalyze = COVARIATES %>%
  ...
  dlply(.(Dx.Tissue.Study), .fun = function(mtd, count){
...
plotdata <- left_join(plotdata, rownameToFirstColumn(COVARIATES, 'SampleID')) %>%
  tidyr::separate(Dx.Tissue.Study, c('Dx','Tissue', 'Study'), sep = '_') %>%
  dplyr::mutate(label = SampleID)
# plotdata$label[!(plotdata$SampleID %in% indToRemove)] = ''

p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=Study, shape=Tissue, size=Age_of_Death))
p <- p + theme_bw() + theme(legend.position="right") + facet_grid(Tissue~.)
# p <- p + geom_text(aes(label= label), size=4, hjust=0)
p

tmp = tidyr::separate(COVARIATES, Dx.Tissue.Study, c('Dx', 'Tissue', 'Study'), sep = '_') %>%
  group_by(Study, Dx, Tissue, Study) %>%
  summarise(count = n()) %>%
  spread(Dx, count)
```
...
COVARIATES.tmp = data.matrix(COVARIATES[,c("Institution", "Gender", "LibraryBatch", "Dx.Tissue.Study")])
...
# Eucledian tree based analysis
COVARIATES.tmp = data.matrix(COVARIATES[,c('Dx.Tissue.Study','Institution')])
COVARIATES.tmp[is.na(COVARIATES.tmp)] = 0
...
WGCNA::plotDendroAndColors(tree, 
                           ...
                           abHeight = 0.80, 
                           main = "Sample dendrogram",
                           groupLabels = colnames(COVARIATES.tmp[,c('Dx.Tissue.Study','Institution')]))
```
...
# Fit contrast
contrast = makeContrasts(contrasts=c("Dx.Tissue.StudySCZ_DLPFC_CMC_-Dx.Tissue.StudyControl_DLPFC_CMC_",
                                     "Dx.Tissue.StudySCZ_DLPFC_HBCC_-Dx.Tissue.StudyControl_DLPFC_HBCC_",
                                     "Dx.Tissue.StudySCZ_ACC_CMC_-Dx.Tissue.StudyControl_ACC_CMC_"),
...
COVARIATES$Dx.Tissue.Study.Gender = paste0(COVARIATES$Dx.Tissue.Study,
                                           COVARIATES$Gender) %>%
  factor()
...
contrast = makeContrasts(contrasts=
                           c("Dx.Tissue.Study.GenderSCZ_DLPFC_CMC_Female-Dx.Tissue.Study.GenderControl_DLPFC_CMC_Female",
                             ...
                             "Dx.Tissue.Study.GenderSCZ_ACC_CMC_Female-Dx.Tissue.Study.GenderControl_ACC_CMC_Female",
                             "Dx.Tissue.Study.GenderSCZ_ACC_CMC_Male-Dx.Tissue.Study.GenderControl_ACC_CMC_Male"),
                         levels = colnames(ADJUSTED.FIT$coefficients))
FIT.CONTR = contrasts.fit(ADJUSTED.FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
...
# Code
CODE <- Folder(name = "CMC and HBCC - ACC - DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN", parentId = parentId)
