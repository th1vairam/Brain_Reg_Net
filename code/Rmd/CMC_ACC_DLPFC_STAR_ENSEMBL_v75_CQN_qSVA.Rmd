---
title: "Covariate analysis of CMC RNASeq data reprocessed with STAR-feature counts workflow with Ensembl v75 (ACC and DLPFC with CQN and qSVA normlaisation)"
...
knit2synapse::knitToFolderEntity(file = "./CMC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_qSVA.Rmd",
                                 parentId = "syn7501880",
                                 entityName = 'ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN - qSVA')
```
...
thisFileName <- 'CMC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_qSVA.Rmd'
...
# Get quality surrogate variables
qSV_DEG_MAT_DLPFC_ID = 'syn10284943'
ALL_USED_IDs[length(ALL_USED_IDs)+1] = qSV_DEG_MAT_DLPFC_ID
qSV_DEG_MAT_DLPFC = downloadFile(qSV_DEG_MAT_DLPFC_ID)

qSV_DEG_MAT_ACC_ID = 'syn10284942'
ALL_USED_IDs[length(ALL_USED_IDs)+1] = qSV_DEG_MAT_ACC_ID
qSV_DEG_MAT_ACC = downloadFile(qSV_DEG_MAT_ACC_ID)

qSV_DEG_MAT = rbindlist(list(qSV_DEG_MAT_DLPFC, qSV_DEG_MAT_ACC), use.names = T, fill = T)
tmp = t(qSV_DEG_MAT[,-(1)])
colnames(tmp) = qSV_DEG_MAT$V1
qSV_DEG_MAT = tmp[,colnames(tmp) %in% colnames(COUNT)]
rownames(qSV_DEG_MAT) = rownames(tmp)

qSV = sva::qsva(qSV_DEG_MAT)
colnames(qSV) = paste0('qSV', 1:dim(qSV)[2])

METADATA = METADATA %>%
  left_join(qSV %>% rownameToFirstColumn('SampleID'))
```
...
writeLines('Remove samples missing qSVs')
writeLines(paste('Following',length(METADATA$SampleID[is.na(METADATA$qSV1)]), 'samples are removed:'))
writeLines(paste(METADATA$SampleID[is.na(METADATA$qSV1)], collapse = ', '))
METADATA <- METADATA  %>% dplyr::filter(!is.na(qSV1))
```
...
ContCovariates <- c("Age_of_Death", "PMI", "RIN", "MappedReads", "IntragenicRate", "IntronicRate", "IntergenicRate",
                    ...
                    "PercentAligned", "EV.1", "EV.2", "EV.3", "EV.4", "EV.5", "qSV1", "qSV2", "qSV3")
...
```
### Normalisation
Based on different model selection methodology (as seen here syn8468116, syn9772932, and syn9774491), following variables are used as covariates: Institution, Gender, RIN, and IntronicRate, with qSV 1 to 3
...
postAdjustCovars = c('Dx.Tissue', 'Institution', 'Gender', 'RIN', 'IntronicRate', 'qSV1', 'qSV2', 'qSV3')
...
postAdjustCovars = c('Dx.Tissue.Gender', 'Institution', 'RIN', 'IntronicRate', 'qSV1', 'qSV2', 'qSV3')
...
postAdjustCovars = c('Tissue', 'Dx', 'Institution', 'Gender', 'RIN', 'IntronicRate', 'qSV1', 'qSV2', 'qSV3')

# Post adjusted design matrix
DESIGN2 = getDesignMatrix(COVARIATES[,postAdjustCovars,drop=F],Intercept = F)$design
...
# Fit contrast
contrast = makeContrasts(contrasts=c("TissueACC-TissueDLPFC",
                                     "DxSCZ-DxControl",
                                     "GenderFemale"),
...
all.annotations = list(
  ...
  normalizationType	= 'qSVA'
)

# Code
CODE <- Folder(name = "ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN - qSVA", parentId = parentId)
