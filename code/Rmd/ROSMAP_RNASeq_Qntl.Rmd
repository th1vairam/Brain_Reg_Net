...
knit2synapse::knitToFolderEntity(file = "./ROSMAP_RNASeq.Rmd",
                                 parentId ="syn5570291",
                                 entityName = 'ROSMAP RNASeq Qntl (Based on cogdx)')
```
...
thisFileName <- 'ROSMAP_RNASeq_Qntl.Rmd'
...
cr = cor(t(RESIDUAL.GENE_EXPRESSION[, !(colnames(RESIDUAL.GENE_EXPRESSION) %in% c('ensembl_gene_id','position'))]))
hist(cr, main = 'Distribution of correlation between genes', xlab = 'Correlation')
```
...
# Code
CODE <- Folder(name = "ROSMAP RNASeq Qntl (Based on cogdx)", parentId = parentId)
CODE <- synStore(CODE)
