...
knit2synapse::knitToFolderEntity(file = "./MSSM_RNASeq_cdr_Reprocessed.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB Reprocessed RNASeq (based on CDR)')
```
...
thisFileName <- 'MSSM_RNASeq_cdr_Reprocessed.Rmd'
...
rownames(METADATA) = METADATA$OldBarcode
...
rownames(COVARIATES) <- METADATA$OldBarcode
...
# AgeAtDeath
p[[2]] = ggplot(METADATA, aes(x = as.factor(CDR), y = as.numeric(AOD), fill = as.factor(BrainRegion))) + geom_boxplot()
p[[2]] = p[[2]] + ggtitle('AgeOfDeath') + theme(legend.position = 'top')

# Mean Plaque
p[[3]] = ggplot(METADATA, aes(x = as.factor(CDR), y = as.numeric(PlaqueMean), fill = as.factor(BrainRegion))) + geom_boxplot()
p[[3]] = p[[3]] + ggtitle('PlaqueMean') + theme(legend.position = 'top')

# NP.1
p[[4]] = ggplot(METADATA, aes(x = as.factor(CDR), y = as.numeric(NP.1), fill = as.factor(BrainRegion))) + geom_boxplot()
...
# Code
CODE <- Folder(name = "MSBB Reprocessed RNASeq (based on CDR)", parentId = parentId)
CODE <- synStore(CODE)
