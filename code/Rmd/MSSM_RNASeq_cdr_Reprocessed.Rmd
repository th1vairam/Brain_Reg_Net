...
tmp1 = t(COUNT[YChromGenes$Gene.ID,]) %>%
  rowSums %>% rownameToFirstColumn('SampleID') %>%
  dplyr::rename(CPM.Sum = DF) %>%
  left_join(COVARIATES %>% 
              rownameToFirstColumn('SampleID'))

ggplot(tmp1, aes(x = SEX, y= log2(CPM.Sum))) + geom_boxplot()
...
tmp1 = t(COUNT[XChromGenes$Gene.ID,]) %>%
  rowSums %>% rownameToFirstColumn('SampleID') %>%
  dplyr::rename(CPM.Sum = DF) %>%
  left_join(COVARIATES %>% 
              rownameToFirstColumn('SampleID'))
