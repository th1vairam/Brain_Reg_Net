...
Genes that are differentially expressed at an FDR <= 0.05 are
```{r diff.exp2, fig.height=15, fig.width=13, eval = F}
...
# Get design matrix
DESIGN = getDesignMatrix(COVARIATES[, c('BrainRegion.Diagnosis.Gender', setdiff(adjust.covars, 'Gender')), drop = F], Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
cntr = c()
for (region in c('DLPFC', 'FP', 'STG', 'PHG', 'IFG', 'CER', 'TCX')){
  tmp = combn(paste0('BrainRegion.Diagnosis.Gender', region, '_', c('AD', 'CONTROL')),2)
  tmp1 = combn(paste0())
  cntr = c(cntr, apply(tmp, 2, paste, collapse = '-'))
}
...
```{r diff.exp3, fig.height=15, fig.width=13, eval = F}
# Get design matrix
