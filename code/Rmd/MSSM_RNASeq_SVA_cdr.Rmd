---
title: "Covariates analysis of MSBB RNASeq (with SVA)"
...
knit2synapse::knitToFolderEntity(file = "./MSSM_RNASeq_SVA_cdr.Rmd",
                                 parentId ="syn5570248",
                                 entityName = 'MSBB RNASeq (based on CDR with SVA)')
```
...
thisFileName <- 'MSSM_RNASeq_SVA_cdr.Rmd'
...
METADATA$BrainRegion.CDR = paste(METADATA$BrainRegion, METADATA$CDR, sep = '.')

# Select covariates of interest
FactorCovariates <- c('RACE', 'SEX', 'BrainRegion.CDR', 'Batch', 'BB')
...
# RIN
p = list()
p[[1]] = ggplot(METADATA, aes(x = CDR, y = RIN, fill = BrainRegion)) + geom_boxplot()
p[[1]] = p[[1]] + ggtitle('RIN') + theme(legend.position = 'top')

# AgeAtDeath
p[[2]] = ggplot(METADATA, aes(x = CDR, y = as.numeric(AOD), fill = BrainRegion)) + geom_boxplot()
p[[2]] = p[[2]] + ggtitle('AgeOfDeath') + theme(legend.position = 'top')

# Mean Plaque
p[[3]] = ggplot(METADATA, aes(x = CDR, y = as.numeric(PlaqueMean), fill = BrainRegion)) + geom_boxplot()
p[[3]] = p[[3]] + ggtitle('PlaqueMean') + theme(legend.position = 'top')

# NP.1
p[[4]] = ggplot(METADATA, aes(x = CDR, y = as.numeric(NP.1), fill = BrainRegion)) + geom_boxplot()
p[[4]] = p[[4]] + ggtitle('NP.1') + theme(legend.position = 'top')

multiplot(plotlist = p, cols = 2)

# Sex
vcd::mosaic(~ SEX + CDR + BrainRegion, data = METADATA)
```
...
p <- p + geom_point(aes(color=BrainRegion.CDR, shape=SEX, size=PMI))
...
postAdjustCovars = c('BrainRegion.CDR', 'RIN', 'PMI', 'AOD', 'Batch') # Based on a separate analysis
...
```{r sva}
MODEL0 = DM1$design[,-grep('BrainRegion.CDR',colnames(DM1$design))] # Get null model by removing variable of interest
MODEL1 = DM1$design

## Calculate the number of surrogate variables
NUM.SV = num.sv(VOOM.ADJUSTED.GENE_EXPRESSION$E, MODEL1, method = 'be', seed = 123456)
...
Calculate weighted residuals and add back "BrainRegion.CDR" to the residuals
```{r varsToAddBack}
# Add variable of interest back to the residuals
varsToAddIn = grep("BrainRegion.CDR", colnames(DM1$design), value = T)
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=factor(BrainRegion.CDR),size=as.numeric(PMI)))
...
names(DE) = gsub('BrainRegion.CDR','',colnames(contrast))
...
# Code
CODE <- Folder(name = "MSBB RNASeq (based on CDR with SVA)", parentId = parentId)
CODE <- synStore(CODE)

# Expression
write.table(RESIDUAL.GENE_EXPRESSION, file='MSSMexpression.tsv', sep = '\t', row.names=F, quote=F)
obj = File('MSSMexpression.tsv', 
           name = 'Voom Adjusted Weighted Residual Expression (BrainRegion.CDR added)', 
