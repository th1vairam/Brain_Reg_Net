...
settings.txt[grep('ssDualKnockouts', settings.txt)] = "ssDualKnockouts = 0"
settings.txt[grep('ssMultifactorial', settings.txt)] = "ssMultifactorial = 0"
settings.txt[grep('tsDREAM4TimeSeries', settings.txt)] = "tsDREAM4TimeSeries = 0"
settings.txt[grep('simulateSDE', settings.txt)] = "simulateSDE = 1"

netSize = c(a = 1e2, b = 1500)
for (net in names(netSize)){
  # Number of minimum regulators
  minReg = max(5, round(netSize[[net]]*0.3))
  ...
  # Extract 1 subnetworks of size n from ecoli data
  system(paste('java','-jar','../../../gnw/gnw-3.1.2b.jar',
                 ...
                 '--random-seed','--greedy-selection','--num-subnets=1',
  ...
  for (fnames in FNames){
    ...
    extractData <- function(pat,allfiles, file.name){
      if (length(grep(paste0(file.name,pat),allfiles)) != 0)
        return(read.table(allfiles[grep(paste0(file.name,pat),allfiles)], header=T, sep = '\t', quote=''))
      else
        return(NULL)
    }
