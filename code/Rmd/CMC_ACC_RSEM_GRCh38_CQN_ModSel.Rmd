---
title: "Covariate analysis of CMC RNASeq data reprocessed with RSEM-feature counts workflow with GRCh38 (ACC with CQN normlaisation)"
...
knit2synapse::createAndKnitToFolderEntity(file = "./CMC_ACC_RSEM_GRCh38_CQN_ModSel.Rmd",
                                          parentId ="syn9846623",
                                          folderName = 'ACC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model - Kelsey')
```
...
activityDescription = 'Covariate analysis of RSEM aligned effective counts with GRCh38 with CQN normalisation (ACC)';

thisFileName <- 'CMC_ACC_RSEM_GRCh38_CQN_ModSel.Rmd'

# Github link
thisRepo <- getRepo(repository = "kelshmo/Brain_Reg_Net", ref="branch", refName='CMC')
...
# Download gene lengths (ACC)
GENE.LEN = downloadFile('syn16783573') %>%
...
METADATA_QC_ACC_ID = 'syn16816488' 
ALL_USED_IDs[length(ALL_USED_IDs)+1] = METADATA_QC_ACC_ID
METADATA_QC_ACC = downloadFile_version(METADATA_QC_ACC_ID, version = 7) %>%
  dplyr::select(Individual_ID, Institution, Reported_Gender, Sex, Ethnicity, Age_of_Death, `PMI_(in_hours)`, Dx,
                Sample_RNA_ID, one_of('rnaSeq_isolation:Exclude?', 'rnaSeq_isolation:RIN',
                                      ...
                                      'rnaSeq_report:Flowcell_Batch', 'rnaSeq_report:Exclude?', 'rnaSeq_report:Mapped_Reads',
                                      'rnaSeq_report:Intragenic_Rate','rnaSeq_report:Intronic_Rate',
                                      'rnaSeq_report:Intergenic_Rate','rnaSeq_report:Genes_Detected',
                                      'rnaSeq_report:Expression_Profiling_Efficiency', 'rnaSeq_report:rRNA_Rate',
                                      'rnaSeq_report:Total_Reads', 'rnaSeq_report:Alignment_Rate'))
```

### Data preprocessing
```{r preprocess.data}
# Merge metadata
METADATA = METADATA_QC_ACC %>%
  ...
  dplyr::rename(PMI = `PMI_(in_hours)`,
                ...
                TotalReads = `rnaSeq_report:Total_Reads`, AlignmentRate = `rnaSeq_report:Alignment_Rate`) %>%
...
FILT <- REPORTED.GENDER.COUNTS %>% 
  ...
  select(-one_of("chromosome_name")) %>% 
  tidyr::spread(FILT, key = gene_id, value = `counts(log)`) %>% 
...
female.Y <- filter(REPORTED.GENDER.COUNTS, chromosome_name == "Y" & `Reported Gender` == "Female") %>% 
  filter(`counts(log)` > 1 | `counts(log)` != "-Inf")

COUNTS.sum <- REPORTED.GENDER.COUNTS %>%
  filter(`counts(log)` != "-Inf") %>% 
  group_by(SampleID, chromosome_name, `Reported Gender`) %>% 
  summarize(`counts(log)sum` = sum(`counts(log)`)) %>% 
  ungroup() %>% 
  group_by(SampleID) %>% 
  mutate(`ratio` = `counts(log)sum`/sum(`counts(log)sum`))
```
...
FactorCovariates <- c('Individual_ID', "Institution", "Reported_Gender", "LibraryBatch", "Dx", "FlowcellBatch")
...
# t <- data.frame(TEMP) %>% 
#   mutate(gene_id = rownames(TEMP))
# REPORTED.GENDER.COUNTS = GENE.GC.CONT %>% 
#   left_join(t) %>%
#   dplyr::select(-one_of("percentage_gene_gc_content")) %>%
#   filter(chromosome_name == "X" |chromosome_name == "Y") %>% 
#   tidyr::gather(key = item, value = value, -c(gene_id, chromosome_name)) %>%
#   rename(`CPM(log)`= value) %>% 
#   rename(SampleID = item) %>%
#   left_join(METADATA[,c("SampleID", "Reported_Gender")]) %>% 
#   rename(`Reported Gender` = Reported_Gender)
...
# 
# p = list()
# p[[1]] = ggplot(filter(REPORTED.GENDER.COUNTS, chromosome_name == "X"), aes(x = `Reported Gender`, y = `CPM(log)`)) + geom_boxplot()
# p[[1]] = p[[1]] + ggtitle('X') + my.theme
# p[[2]] = ggplot(filter(REPORTED.GENDER.COUNTS, chromosome_name == "Y"), aes(x = `Reported Gender`, y = `CPM(log)`)) + geom_boxplot()
# p[[2]] = p[[2]] + ggtitle('Y') + my.theme
#multiplot(plotlist = p, cols = 2)
```
...
p[[1]] <- p[[1]] + geom_point(aes(color=Institution, shape=Dx, size=Age_of_Death))
p[[1]] <- p[[1]] + theme_bw() %+replace% theme(legend.position="top") + guides(colour = guide_legend(nrow = 2),
...
p[[2]] <- ggplot(plotdata, aes(MDS1,MDS2))
p[[2]] <- p[[2]] + geom_point(aes(color=Institution, shape=Dx, size=Age_of_Death))
p[[2]] <- p[[2]] + theme_bw() %+replace% theme(legend.position="top") + guides(colour = guide_legend(nrow = 2),
                                                                               shape = guide_legend(nrow = 2),
                                                                               size = guide_legend(nrow = 2))
...
phase1.covars = c('Institution', 'Reported_Gender', 'PMI', 'Age_of_Death', 'RIN', 'RIN2', 'EV.1', 'EV.2', 'EV.3', 'EV.4', 'EV.5')
...
# Code
CODE <- Folder(name = "ACC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)
