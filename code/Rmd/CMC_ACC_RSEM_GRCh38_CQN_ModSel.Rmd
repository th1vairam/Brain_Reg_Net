...
GENE.LEN = downloadFile('syn16783573') %>%
  ...
  ungroup() %>% data.frame()
ALL_USED_IDs = c(ALL_USED_IDs, 'syn16783573')
...
METADATA_QC_ACC = downloadFile_version(METADATA_QC_ACC_ID, version = 8) %>% 
...
mart <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", host = "oct2018.archive.ensembl.org", dataset = "hsapiens_gene_ensembl")#change to specific ensembl archive....host 
...
To summarize NAN values:
sapply(METADATA, function(x) sum(is.nan(x)))
...
LOG.CPM = cpm(PROCESSED_COUNTS$filteredExprMatrix$counts, log = T) %>%
  ...
  tidyr::gather(sample_id, value, -gene_id) %>%
  group_by(gene_id) %>%
  nest() %>%
  tibble::deframe() %>%
  future_map(function(x){
    mn = mean(x$value, na.rm = T)
    std.dev = sd(x$value, na.rm = T)
    
    x[which(x$value <= mn-3*std.dev), 'value'] = NA 
    x[which(x$value >= mn+3*std.dev), 'value'] = NA 
    
    return(x)
  }, .progress = T) %>%
  bind_rows(.id = 'gene_id') %>%
  tidyr::spread(sample_id, value) %>%
  tibble::column_to_rownames(var = 'gene_id') %>%
  as.data.frame()

# Assign zero values to NAs
LOG.CPM = LOG.CPM[rownames(PROCESSED_COUNTS$filteredExprMatrix$counts), colnames(PROCESSED_COUNTS$filteredExprMatrix$counts)]
...
CQN.GENE_EXPRESSION = cqn(round(NEW.COUNTS[,c(1:190, 192:476)]), 
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(color=Institution, shape=Dx, size=Age_of_Death))
p <- p + theme_bw() %+replace% theme(legend.position="top") + guides(colour = guide_legend(nrow = 2),
