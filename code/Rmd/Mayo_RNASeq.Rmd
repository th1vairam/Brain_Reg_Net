...
knit2synapse::knitToFolderEntity(file = "./Mayo_RNASeq.Rmd",
                                 parentId = "syn5570325",
                                 entityName = 'UFL_Mayo_ISB RNASeq')
```
...
tmp = base::gsub('_','\\\\_',tmp)
writeLines(tmp)
tmp = paste('Samples in cluster 1 are', paste(sampleTrees$labels[tree == 1], collapse = ','))
tmp = base::gsub('_','\\\\_',tmp)
writeLines(tmp)
tmp = paste('Samples in cluster 3 are', paste(sampleTrees$labels[tree == 3], collapse = ','))
tmp = base::gsub('_','\\\\_',tmp)
...
# Code
CODE <- Folder(name = 'UFL_Mayo_ISB RNASeq', parentId = parentId)
CODE <- synStore(CODE)
