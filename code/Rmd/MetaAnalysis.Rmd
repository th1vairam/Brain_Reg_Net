...
download.file <- function(id){ tmp = fread(synGet(id)@filePath, data.table=F, header=T);  }
...
rownames(Adj.P.Value)[which(rowSums(Adj.P.Value<=0.05)==7)]

writeLines('Genes that are differentially expressed at a FDR <= 0.05 in 6/8 datasets')
rownames(Adj.P.Value)[which(rowSums(Adj.P.Value<=0.05)==6)]
```
...
FILE_OBJ = synStore(FILE_OBJ, 
                    used = as.character(c(EXP_ID, COV_ID)),
                    executed = thisFile,
