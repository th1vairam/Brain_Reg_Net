...
covariates$MSBB = covariates$MSBB %>%
  ...
  dplyr::filter(Diagnosis %in% c('SD', 'ND')) %>%
  dplyr::mutate(Study = 'MSSM',
                Dx = factor(Diagnosis, levels = c('ND','SD')),
...
tmp2 = dplyr::select(tmp1, ComparisonName, GeneSetName, Odds.Ratio) %>%
  tidyr::spread(ComparisonName,Odds.Ratio)
