...
tmp1 = rbindlist(diff.exp, use.names = T, fill = T) %>%
  dplyr::filter(gene.id %in% Reduce(c, diff.exp.gl)) %>%
...
tmp = tmp1 %>%
  ...
  filter(count >= 5) 
writeLines(paste('Differentially expressed genes in any 5 brain regions at an FDR of 0.05 and fold change of 1.2 are',
...
pl = list()
p = ggplot(tmp1 %>% 
             filter(hgnc_symbol %in% tmp.eff.sz$hgnc_symbol[tmp.eff.sz$es>2.6]), 
...
p = p + theme(legend.position = 'top') + scale_fill_manual(values = c('grey', 'red'))
pl[[1]] = p
...
setsToTest = rbindlist(diff.exp, use.names = T, fill = T) %>%
  filter(adj.P.Val <= 0.05, Direction != 'NONE') %>%
  dlply(.(Region, Direction), .fun = function(x){
...
backgroundGenes = unique(rbindlist(diff.exp, use.names = T, fill = T) %>% 
                           dplyr::select(hgnc_symbol) %>%
                           unlist)
