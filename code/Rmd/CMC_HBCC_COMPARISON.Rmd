...
knit2synapse::knitToFolderEntity(file = "./CMC_HBCC_COMPARISON.Rmd",
                                 parentId = "syn7501880",
...
rep.rate %>%
  ...
  ggplot(aes(x=fdr, y=rep.rate, size = ncount, color = method)) + geom_point() + geom_line() + scale_x_log10() + xlab('FDR') + ylab('Replication Rate (%)')
```
