...
for (pval in seq(0,-4,-1)){
  ...
  de.genes = diff.exp %>%
    dplyr::filter(Comparison %in% c('SCZ_DLPFC_CMC-Control_DLPFC_CMC', 'SCZ_DLPFC_HBCC-Control_DLPFC_HBCC')) %>%
    dplyr::mutate(Direction = factor(sign(logFC), levels = c('-1','1'), labels = c('-1' = 'DOWN','1' = 'UP'))) %>%
    plyr::dlply(.(Comparison, Model, Direction), .fun = function(x){
      x$ensembl_gene_id[x$adj.P.Val <= 10^pval] %>% unique()
    })
  
  l1 = length(intersect(de.genes$`SCZ_DLPFC_CMC-Control_DLPFC_CMC.MATCHED.DOWN`,
                        de.genes$`SCZ_DLPFC_HBCC-Control_DLPFC_HBCC.MATCHED.DOWN`))
  l2 = length(intersect(de.genes$`SCZ_DLPFC_CMC-Control_DLPFC_CMC.MATCHED.UP`, 
                        de.genes$`SCZ_DLPFC_HBCC-Control_DLPFC_HBCC.MATCHED.UP`))
  l3 = length(unique(Reduce(c, de.genes)))
  matched = (l1+l2)/l3
  n.matched = l1+l2

  rep.rate = c(rep.rate, list(data.frame(fdr = 10^pval, 
                                         wo.qsva = wo.qsva, w.qsva = w.qsva, matched = matched,
                                         n.wo.qsva = n.wo.qsva, n.w.qsva = n.w.qsva, n.matched = n.matched)))
}
rep.rate = rbindlist(rep.rate)
rep.rate %>%
  dplyr::select(fdr, wo.qsva, w.qsva, matched) %>%
  tidyr::gather(method, rep.rate, -fdr) %>%
  dplyr::mutate(rep.rate = rep.rate * 100) %>%
  dplyr::left_join(rep.rate %>%
                     dplyr::select(fdr, n.wo.qsva, n.w.qsva, n.matched) %>%
                     tidyr::gather(method, nOverlap, -fdr) %>%
                     dplyr::mutate(method = gsub('n.','',method))) %>%
  dplyr::filter(fdr <= 1e-1) %>%
  ggplot(aes(x=fdr, y=rep.rate, color = method)) + geom_point(aes(size = nOverlap)) + geom_line() + scale_x_log10() + xlab('FDR') + ylab('Replication Rate (%)') + ggtitle('CMC - HBCC - DLPFC')
```
...
for (pval in c(-1, -1.30103, -2)){
  ...
  tmp = diff.exp %>%
    dplyr::filter(Comparison == 'SCZ_DLPFC_CMC-Control_DLPFC_CMC', 
                  ensembl_gene_id %in% de.genes$`SCZ_DLPFC_HBCC-Control_DLPFC_HBCC`) %>%
    dplyr::select(P.Value)
  ...
  if (dim(tmp)[1] != 0){
    tmp %>%
      qvalue::qvalue(fdr.level = 10^pval)
    rs = rbind(rs,
               data.frame(method = 'matched', ref.data = 'HBCC', rep.data = 'CMC',
                          rep.rate = 1-tmp$pi0, 
                          ncount = sum(tmp$significant)))
  } else {
    rs = rbind(rs,
  ...
  tmp = diff.exp %>%
    dplyr::filter(Comparison == 'SCZ_DLPFC_HBCC-Control_DLPFC_HBCC', 
                  ensembl_gene_id %in% de.genes$`SCZ_DLPFC_CMC-Control_DLPFC_CMC`) %>%
    dplyr::select(P.Value) %>%
    qvalue::qvalue(fdr.level = 10^pval)
  rs = rbind(rs,
             data.frame(method = 'matched', ref.data = 'CMC', rep.data = 'HBCC',
                        rep.rate = 1-tmp$pi0, 
                        ncount = sum(tmp$significant)))
