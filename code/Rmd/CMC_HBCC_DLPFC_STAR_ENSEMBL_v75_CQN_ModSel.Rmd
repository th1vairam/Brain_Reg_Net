...
p = ggpubr::ggarrange(plotlist = p, ncol = 4, nrow = 2) 
p
```
...
tmp = COVARIATES %>%
  dplyr::group_by(Dx.Tissue, Study) %>%
  dplyr::summarise(mn = quantile(Age_of_Death, 0.25), mx = quantile(Age_of_Death, 0.75)) %>%
  dplyr::group_by(Dx.Tissue) %>%
  dplyr::summarise(mn = max(mn), mx = min(mx))
COVARIATES = COVARIATES %>%
  dplyr::filter((Dx.Tissue == tmp$Dx.Tissue[1] & Age_of_Death >= tmp$mn[1] & Age_of_Death <= tmp$mx[1]) |
                  (Dx.Tissue == tmp$Dx.Tissue[2] & Age_of_Death >= tmp$mn[2] & Age_of_Death <= tmp$mx[2]))
...
NOTE: Dx.Tissue.Study is chosen as the primary variable of interest (i.e., covariate selection is conditioned on diagnosis and tissue)

```{r custom.normalisation, results='asis'}
# Primary variable of interest
COVARIATES$Dx.Tissue.Study = paste(COVARIATES$Dx.Tissue, COVARIATES$Study, sep='_') %>%
  factor
postAdjustCovars = c('Dx.Tissue.Study', 'Institution', 'Gender', 'RIN', 'IntronicRate')
...
write.table(all.diff.exp, file = 'CMC_HBCC_DLPFC_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
