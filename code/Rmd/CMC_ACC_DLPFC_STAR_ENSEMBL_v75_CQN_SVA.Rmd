...
indToRemove = c('MSSM_RNA_ACC_BP_24', 
                
                'MSSM_RNA_ACC_375', 'MSSM_RNA_ACC_170', 'MSSM_RNA_ACC_315', 'MSSM_RNA_ACC_250', 'MSSM_RNA_ACC_21', 'MSSM_RNA_ACC_356', 
                'MSSM_RNA_ACC_318', 'MSSM_RNA_ACC_222', 'MSSM_RNA_ACC_220', 'MSSM_RNA_ACC_379', 'MSSM_RNA_PFC_133',
                
                'MSSM_RNA_PFC_139', 'MSSM_RNA_PFC_163', 'MSSM_RNA_PFC_260', 'MSSM_RNA_PFC_299', 'MSSM_RNA_PFC_361', 'MSSM_RNA_PFC_89', 
                'MSSM_RNA_PFC_319', 'MSSM_RNA_PFC_348', 'MSSM_RNA_PFC_354', 'MSSM_RNA_PFC_60', 'PENN_RNA_PFC_71', 'PENN_RNA_PFC_56')
...
Since many covariates are correlated, re-normalising COUNTS with a custom design matrix. 
1. "Dx.Tissue" is chosen as the primary variable of interest (i.e., covariate selection is conditioned on diagnosis and tissue)
2.  Model:
    Gx = Dx.Tissue + Institution + Gender + IntronicRate + RibozeroBatch + PMI + RIN + TranscriptsDetected +
    Age\_of\_Death + IntergenicRate + MappedReads + PercentAligned + (1/Individual\_ID)    
```{r custom.normalisation, results='asis'}
# Primary variable of interest
postAdjustCovars = c("Dx.Tissue", "Institution", "Gender", "IntronicRate", "RibozeroBatch", "PMI", "RIN",
                     "TranscriptsDetected", "Age_of_Death", "IntergenicRate", "MappedReads", "PercentAligned");
...
RESIDUAL.SVA.GENE_EXPRESSION = residuals.MArrayLM(ADJUSTED.FIT, VOOM.GENE_EXPRESSION$E)
...
RESIDUAL.GENE_EXPRESSION = residuals.MArrayLM(VOOM.SVA.FIT,
...
varsToAddIn = grep("Dx.Tissue", colnames(VOOM.SVA.FIT$coefficients), value = T)
...
# Post adjusted design matrix
DESIGN = getDesignMatrix(cbind(COVARIATES[,postAdjustCovars,drop=F], SURR.VAR),
                               Intercept = F)
...
SURR.VAR_OBJ = File('CMC_ACC_DLPFC_SurrVar.tsv', name = 'Surrogate Variables', parentId = CODE$properties$id)
annotations(SURR.VAR_OBJ) = annotations(CODE)
annotations(SURR.VAR_OBJ)$dataSubType = 'SurrogateVariables'
...
annotations(nEXP_OBJ)$dataSubType = 'residualeQTLAnalysis'
...
|  Residual Expression (for eQTL analysis)    |  `r nEXP_OBJ$properties$id`      |
