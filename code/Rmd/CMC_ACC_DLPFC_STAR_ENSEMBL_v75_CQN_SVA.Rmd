---
title: Covariate analysis of CMC RNASeq data reprocessed with STAR-feature counts workflow with Ensembl v75 (ACC and DLPFC with CQN + SVA normlaisation)
...
NOTE: Dx.Tissue is chosen as the primary variable of interest (i.e., covariate selection is conditioned on diagnosis and tissue), and Individual_ID is chosen as the random effect
```{r custom.design}
...
cnts = NEW.COUNTS
cnts[is.na(cnts)] = 0
VOOM.GENE_EXPRESSION = voom(cnts, design = DESIGN)
...
SURR.VAR = irwsva.build(CQN.GENE_EXPRESSION$E.no.na, MODEL1, MODEL0, n.sv = NUM.SV, B = 30, tol = 1e-20)$sv
...
# Estimate voom weights
VOOM.GENE_EXPRESSION1 = voom(cnts, design = DESIGN1)
VOOM.GENE_EXPRESSION1$E = CQN.GENE_EXPRESSION$E
correlation <- parallelDuplicateCorrelation(VOOM.GENE_EXPRESSION1, 
                                            block = COVARIATES$Individual_ID,
                                            method = 'lmer')

# Re-calculate voom weights with correlation of random effects
VOOM.GENE_EXPRESSION1 = voom(cnts,   
...
correlation <- parallelDuplicateCorrelation(VOOM.GENE_EXPRESSION2, 
                                            block = COVARIATES$Individual_ID,
                                            method = 'lmer')

# Re-calculate voom weights with correlation of random effects
VOOM.GENE_EXPRESSION2 = voom(cnts,   
