...
metadataVarOfInterest = c("Individual ID", "Institution", "Gender", "Age of Death", "PMI (in hours)", "Dx", 
                          ...
                          "DLPFC_RNA_report: Library Batch", "DLPFC_RNA_report: Ribozero Batch", "DLPFC_RNA_report: Flowcell Batch",
...
METADATA = METADATA %>%
  dplyr::select(Individual_ID, Institution, Gender, Age_of_Death, PMI__in_hours_, Dx, 
                ...
                DLPFC_RNA_isolation__RIN, DLPFC_RNA_report__Exclude_, DLPFC_RNA_report__Ribozero_Batch, 
                DLPFC_RNA_report__Library_Batch, DLPFC_RNA_report__Flowcell_Batch) %>%
...
writeLines('Some control samples with Age of death less than 21 are removed')
writeLines(paste('Following',length(METADATA$SampleID[METADATA$Age_of_Death <= 21]), 'samples are removed:'))
writeLines(paste(METADATA$SampleID[METADATA$Age_of_Death <= 21], collapse = ', '))
METADATA <- METADATA  %>% dplyr::filter(Age_of_Death >= 21)
```
...
FactorCovariates <- c("Gender", "Dx.Tissue")
...
# Fit contrast
contrast = makeContrasts(contrasts=c("Dx.TissueBP_DLPFC-Dx.TissueControl_DLPFC",
...
tmp = de.hbcc %>%
  ...
  dplyr::select(ensembl_gene_id, Comparison, Tissue1, logFC) %>%
  group_by(Comparison, Tissue1) %>%
  dplyr::summarise(FDR_0_05_UP = length(unique(ensembl_gene_id[logFC > 0])),
                   FDR_0_05_DOWN = length(unique(ensembl_gene_id[logFC < 0])))

tmp1 = de.hbcc %>%
  dplyr::filter(adj.P.Val <= 0.05, abs(logFC) >= log2(1.2)) %>%
  dplyr::select(ensembl_gene_id, Comparison, Tissue1, logFC) %>%
  group_by(Comparison, Tissue1) %>%
  dplyr::summarise(FDR_0_05_FC_1.2_UP = length(unique(ensembl_gene_id[logFC > log2(1.2)])),
                   FDR_0_05_FC_1.2_DOWN = length(unique(ensembl_gene_id[logFC < -log2(1.2)])))
...
pl = list()
pl[[1]] = ggplot(DE, aes(x = log10(as.numeric(Length)), y = logFC, color = Direction)) + geom_point() 
pl[[1]] = pl[[1]] + geom_smooth(method = 'loess', inherit.aes = FALSE, aes(x = log10(as.numeric(Length)), y = logFC))
...
diffexp.genes = de.hbcc %>%
  dplyr::mutate(Direction = sign(logFC),
                Direction = factor(Direction, c(-1,1), c('-1' = 'DOWN', '1' = 'UP'))) %>%
  dlply(.(Comparison, Tissue1, Direction), .fun = function(x){
...
enrichment.results = plyr::llply(diffexp.genes, .fun = function(x, genesetsToTest, backgroundGenes){
...
# Code
CODE <- Folder(name = "HBCC - DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN (Model Selection)", parentId = parentId)
