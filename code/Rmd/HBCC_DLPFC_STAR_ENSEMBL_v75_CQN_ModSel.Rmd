...
knit2synapse::knitToFolderEntity(file = "./HBCC_DLPFC_STAR_ENSEMBL_v75_CQN_ModSel.Rmd",
                                 parentId ="syn9846623",
                                 folderName = 'HBCC - DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN (Model Selection)')
```
...
thisFileName <- 'HBCC_DLPFC_STAR_ENSEMBL_v75_CQN_ModSel.Rmd'
...
downloadFile <- function(id, sep){
  tmp = data.table::fread(synGet(id)@filePath,  data.table = F, header = T, sep = sep)
}

# Download reprocessed counts (DLPFC)
COUNT = downloadFile('syn8465067', sep = '\t') %>%
  as.data.frame()
ALL_USED_IDs = 'syn8465067'
...
METADATA_CLINICAL = downloadFile('syn2279441', sep = ',') %>%
  dplyr::rename(Individual_ID = `Individual ID`)
ALL_USED_IDs = c(ALL_USED_IDs, 'syn2279441')

# Get RNA alignment report from synapse
METADATA_ALG_REP_DLPFC = downloadFile('syn2279446', sep = ',') %>%
  dplyr::select(`Exclude?`, `Sample RNA ID`, `Ribozero Batch`, `Library Batch`, `Flowcell Batch`) %>%
  dplyr::rename(Exclude = `Exclude?`,
                SampleID = `Sample RNA ID`,
                LibraryBatch = `Library Batch`, 
                RibozeroBatch = `Ribozero Batch`, 
                FlowcellBatch = `Flowcell Batch`) %>%
  distinct()
ALL_USED_IDs = c(ALL_USED_IDs, 'syn2279446')

# Get RNA qc metrics from synapse
METADATA_RNASEQ_QC = downloadFile('syn8539839', sep = ',') %>%
  dplyr::select(V1, Sample, Total_Purity_Filtered_Reads_Sequenced, X5prime_Norm, Mean_Per_Base_Cov.,  
...
ALL_USED_IDs = c(ALL_USED_IDs, 'syn8539839')

# Get RNASeq isolation report
METADATA_RNA_ISOLATION = downloadFile('syn2279445', sep = ',') %>%
  dplyr::select(`Sample RNA ID`, RIN) %>%
  dplyr::rename(SampleID = `Sample RNA ID`)
ALL_USED_IDs = c(ALL_USED_IDs, 'syn2279445')

# Get individual id mapping
METADATA_IID = downloadFile('syn12979139', sep = '\t') %>%
  dplyr::select(Data_ID, IID_SAMPLE, label) %>%
  dplyr::rename(Individual_ID = IID_SAMPLE,
                SampleID = Data_ID) %>%
  distinct()
ALL_USED_IDs = c(ALL_USED_IDs, 'syn12979139')

# Merge metadata
METADATA = dplyr::inner_join(METADATA_ALG_REP_DLPFC, METADATA_RNASEQ_QC) %>%
  dplyr::inner_join(METADATA_IID) %>%
  dplyr::inner_join(METADATA_CLINICAL) %>%
  dplyr::inner_join(METADATA_RNA_ISOLATION) %>%
  dplyr::select(SampleID, Individual_ID, label, 
                LibraryBatch, RibozeroBatch, FlowcellBatch,
                Dx, Gender, `PMI (in hours)`, `Age of Death`, RIN,
                TotalReads, X5prime_Norm, Mean_Per_Base_Cov.,
                Mean_CV, Fragment_Length_StdDev, Mapped_Pairs, End_1_Mismatch_Rate, End_2_Mismatch_Rate, 
                Mapped, IntragenicRate, ExonicRate, IntergenicRate, IntronicRate, rRNARate, GenesDetected,
                TranscriptsDetected, ExpProfEfficiency, PercentMapped) %>%
  dplyr::rename(PMI = `PMI (in hours)`,
                Age_of_Death = `Age of Death`) %>%
  dplyr::mutate_at(.vars = c('LibraryBatch', 'RibozeroBatch', 'FlowcellBatch', 'Dx', 'Gender'), 
                   .funs = function(x){
                     x = forcats::fct_explicit_na(x, na_level = 'NotAvailable')
                     levels(x)[levels(x) == ''] = 'NotAvailable'
                     return(x)
                   }) %>%
...
fitFixedEffectModel <- function(varsToFit, lcpm, cnt, covars){
  ...
  modelBIC = plyr::ldply(1:dim(lcpm)[1],
                         .fun = function(i, lcpm, designMat, voomWeights){
                           # Fit a linear models for every gene
                           data = cbind(data.frame(Gx = lcpm[i,]),
                                        designMat, 
                                        weight = voomWeights[i,])
                           
                           formula = paste(setdiff(colnames(data), c('Gx', 'weight')), collapse = '+') %>%
                             paste('Gx ~',., collapse = ' ')
                           
                           mdl = stats::lm(formula = formula, 
                                           data = data, 
                                           weights = weight)
                           
                           data.frame(ensembl_gene_id = rownames(lcpm)[i], BIC = BIC(mdl))
                         }, 
                         lcpm, designMat, voomWeights, 
                         .parallel = F,
...
selectCovarsToAdjust <- function(covarsToSelect, adjustCovars, covars, lcpm, cnt, baseModelBIC){
  ...
  while(length(covarsToSelect) != 0){
    allModelBIC = plyr::llply(1:length(covarsToSelect),
                              .fun = function(i, covarsToSelect, adjustCovars, lcpm, cnt, covars){
                                
                                # Fit a mixed effect linear model and get all BIC
                                modelBIC = fitFixedEffectModel(c(adjustCovars, covarsToSelect[i]),
                                                               lcpm, cnt, covars) %>%
                                  plyr::rename(c('BIC' = covarsToSelect[i]))
                                
                                return(modelBIC)
                              },
                              covarsToSelect, adjustCovars, lcpm, cnt, covars, 
                              .parallel = F,
                              .paropts = list(.export = c('fitFixedEffectModel')))
...
baseModelBIC = fitFixedEffectModel(adjust.covars, 
...
phase1.covars = c('Gender', 'PMI', 'Age_of_Death', 'RIN', 'RIN2')
...
# Iteratively identify covariates in phase I
phase3.covars = c("TotalReads", "X5prime_Norm", "Mean_Per_Base_Cov.", "Mean_CV", "Fragment_Length_StdDev",
                  "Mapped_Pairs", "End_1_Mismatch_Rate", "End_2_Mismatch_Rate", "Mapped", "IntragenicRate",
                  "ExonicRate", "IntergenicRate", "IntronicRate", "rRNARate", "GenesDetected",
                  "TranscriptsDetected", "ExpProfEfficiency", "PercentMapped")
...
# Post adjusted design matrix
DESIGN = getDesignMatrix(droplevels(COVARIATES[,postAdjustCovars,drop=F]),Intercept = F)$design
DESIGN = DESIGN[,linColumnFinder(DESIGN)$indepCols]

# Estimate voom weights
tmp = NEW.COUNTS
tmp[is.na(tmp)] = 0
VOOM.GENE_EXPRESSION = limma::voom(tmp, design = DESIGN)
VOOM.GENE_EXPRESSION$E = CQN.GENE_EXPRESSION$E
...
tmp = NEW.COUNTS
tmp[is.na(tmp)] = 0
VOOM.GENE_EXPRESSION1 = limma::voom(tmp, design = DESIGN1)
...
tmp = NEW.COUNTS
tmp[is.na(tmp)] = 0
VOOM.GENE_EXPRESSION2 = voom(tmp, design = DESIGN2)
...
geneset.files = synapseClient::synQuery('select * from file where parentId == "syn3240583"')
...
genesets.to.test = sapply(geneset.files$file.name, function(dataSource, geneset.files){
  tmp = downloadFile(geneset.files$file.id[geneset.files$file.name == dataSource], sep = '\t') %>%
...
# Code
CODE <- synapseClient::Folder("HBCC - DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN (Model Selection)", 
                              parentId = parentId)
synapseClient::synSetAnnotations(CODE, annotations = all.annotations)
...
COV_OBJ = synapseClient::File('CMC_ACC_DLPFC_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
synapseClient::annotations(COV_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(COV_OBJ)$dataSubType = 'covariates'
COV_OBJ = synapseClient::synStore(COV_OBJ, used = ALL_USED_IDs, activityName = activityName, 
...
COUNT_OBJ = synapseClient::File('CMC_ACC_DLPFC_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
synapseClient::annotations(COUNT_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(COUNT_OBJ)$dataSubType = 'filteredCounts'
COUNT_OBJ = synapseClient::synStore(COUNT_OBJ, activity = synapseClient::synGetActivity(COV_OBJ$properties$id))

# Store logCPM
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_ACC_DLPFC_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = synapseClient::File('CMC_ACC_DLPFC_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
synapseClient::annotations(LCOUNT_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(LCOUNT_OBJ)$dataSubType = 'filteredLCPM'
LCOUNT_OBJ = synapseClient::synStore(LCOUNT_OBJ, activity = synapseClient::synGetActivity(COV_OBJ$properties$id))

# Store cqn offsets
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_ACC_DLPFC_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = synapseClient::File('CMC_ACC_DLPFC_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
synapseClient::annotations(OFFSET_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(OFFSET_OBJ)$dataSubType = 'offset'
OFFSET_OBJ = synapseClient::synStore(OFFSET_OBJ, activity = synapseClient::synGetActivity(COV_OBJ$properties$id))

# Store design matrix
DM1$design %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'CMC_ACC_DLPFC_Design.tsv', sep = '\t', row.names=F, quote=F)
DM_OBJ = synapseClient::File('CMC_ACC_DLPFC_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
synapseClient::annotations(DM_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(DM_OBJ)$dataSubType = 'designMatrix'
DM_OBJ = synapseClient::synStore(DM_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
...
nEXP_OBJ = synapseClient::File('CMC_ACC_DLPFC_netResidualExpression.tsv', 
                               name = 'Normalised, covariates removed residual expression (for network analysis)', 
                               parentId = CODE$properties$id)
synapseClient::annotations(nEXP_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(nEXP_OBJ)$dataSubType = 'residualGeneExpForNetAnlz'
nEXP_OBJ = synapseClient::synStore(nEXP_OBJ, activity = synapseClient::synGetActivity(COV_OBJ$properties$id))

# Store differential expression results
write.table(DE, file = 'CMC_ACC_DLPFC_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = synapseClient::File('CMC_ACC_DLPFC_DiffExpression.tsv', 
                               name = 'Differential Expression Results (Dx.Tissue)', 
                               parentId = CODE$properties$id)
synapseClient::annotations(DEXP_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(DEXP_OBJ)$dataSubType = 'diffExp'
DEXP_OBJ = synapseClient::synStore(DEXP_OBJ, activity = synapseClient::synGetActivity(COV_OBJ$properties$id))

# Store enrichment results
write.table(enrichment.results, file = 'CMC_ACC_DLPFC_EnrichResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = synapseClient::File('CMC_ACC_DLPFC_EnrichResults.tsv', 
                                 name = 'Enrichment Analysis Results (Dx.Tissue)', 
                                 parentId = CODE$properties$id)
synapseClient::annotations(ENRICH_OBJ) = synapseClient::annotations(CODE)
synapseClient::annotations(ENRICH_OBJ)$dataSubType = 'enrichResults'
ENRICH_OBJ = synapseClient::synStore(ENRICH_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
# stopCluster(cl)
```
|  *Results*                                  |  *SynapseID*                     |
|  -----------------------------------------  |   ---------                      |
