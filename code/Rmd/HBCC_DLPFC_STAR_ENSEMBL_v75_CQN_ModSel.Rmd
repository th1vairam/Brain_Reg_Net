...
METADATA_QC = read.table(synGet(METADATA_QC_DLPFC_ID)@filePath, fill = T, header = T, sep = ',', row.names = 1) %>%
  rownameToFirstColumn('Sample.RNA.ID') %>%
  dplyr::select(Sample.RNA.ID, Total_Purity_Filtered_Reads_Sequenced, X5prime_Norm, Mean_Per_Base_Cov.,  
...
writeLines('Some control samples with Age of death less than 17 are removed')
writeLines(paste('Following',length(METADATA$SampleID[METADATA$Age_of_Death <= 17]), 'samples are removed:'))
writeLines(paste(METADATA$SampleID[METADATA$Age_of_Death <= 17], collapse = ', '))
METADATA <- METADATA  %>% dplyr::filter(Age_of_Death >= 17) 
```
...
fitFixedEffectModel <- function(varsToFit, lcpm, cnt, covars){
  ...
  modelBIC = plyr::ldply(1:dim(lcpm)[1],
                         .fun = function(i, lcpm, designMat, voomWeights){
                           ...
                           formula = paste(setdiff(colnames(data), c('Gx')), collapse = '+') %>%
                             paste('Gx ~',., collapse = ' ')
                           
                           mdl = lm(formula = formula, 
                                    data = data, 
                                    subset = 1:dim(data)[1],
                                    weights = wts)
