...
knit2synapse::createAndKnitToFolderEntity(file = "./CMC_DLPFC_RSEM_GRCh38_CQN_ModSel.Rmd",
                                          parentId ="syn9846623",
                                          folderName = 'DLPFC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model')
```
...
```{r sex-specific_expr, fig.height=8, fig.width=8, results='asis', cache = FALSE}
#filter for sex specific chromosomes here. Plot x and y expression separately for males and females 
```
...
#plotMDS of CQN normalized expression data plotMDS(x) where x is any data object which can be coerced to a matrix 
