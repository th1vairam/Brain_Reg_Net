...
primaryVariable <- c("cogdx", "Diagnosis")
FactorCovariates <- c("Batch", "msex", "race", "spanish", "cogdx", "Diagnosis")
...
CQN.GENE_EXPRESSION = cqn(PROCESSED_COUNTS$filteredExprMatrix$counts, 
                          x = GENE.GC[rownames(PROCESSED_COUNTS$filteredExprMatrix$counts), 'percentage_gc_content'],
                          lengths = GENE.LEN[rownames(PROCESSED_COUNTS$filteredExprMatrix$counts), 'gene.length'],
                          lengthMethod = "smooth", 
                          verbose = FALSE)
CQN.GENE_EXPRESSION$E.NA = CQN.GENE_EXPRESSION$y+CQN.GENE_EXPRESSION$offset
CQN.GENE_EXPRESSION$E.zero = CQN.GENE_EXPRESSION$y+CQN.GENE_EXPRESSION$offset

# Set gene counts in specific samples that are deviating 3 sd from other samples to 0
log.mat = apply(CQN.GENE_EXPRESSION$E.zero, 1, function(x){
  mn = mean(x, na.rm = T)
...
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 20){
  ...
  counts = PROCESSED_COUNTS$filteredExprMatrix$counts
  counts[is.na(counts)] = 0
  VOOM.GENE_EXPRESSION = voom(counts, 
                              design=DM1$design, 
                              plot=F,
                              na.rm = T)
  ...
  RESIDUAL.GENE_EXPRESSION$expr.zero[is.na(RESIDUAL.GENE_EXPRESSION$expr.NA)] = 0
...
counts = PROCESSED_COUNTS$filteredExprMatrix$counts
counts[is.na(counts)] = 0
VOOM.WEIGHTS = voom(counts, design=DESIGN$design, plot=F)

# Fit linear model using new weights and new design
FIT = lmFit(CQN.GENE_EXPRESSION$E.NA,
...
COVARIATES$Diagnosis.msex = paste(COVARIATES$Diagnosis, factor(COVARIATES$msex, labels = c('0' = 'FEMALE', '1' = 'MALE')), sep = '.') %>%
  factor
...
VOOM.SVA.WEIGHTS = voom(counts, 
                        design = cbind(MODEL1, SURR.VAR), plot=F)

# Fit linear model using new weights and new design
VOOM.SVA.FIT = lmFit(CQN.GENE_EXPRESSION$E.NA, 
...
# Estimate voom weights for dispersion control
VOOM.NET.WEIGHTS = voom(counts, 
                        design=DESIGN.NET, plot=F)

# Fit linear model using new weights and new design
VOOM.NET.FIT = lmFit(CQN.GENE_EXPRESSION$E.NA,
                     design = DESIGN.NET,
...
# Create seperate acccess list for 90+ data
changeACL <- function(id){
  old.acl = synGetEntityACL(METADATA.CLINICAL_OBJ1@properties$id)
  myaccess = ResourceAccess('3319864', list('UPDATE', 'CHANGE_SETTINGS', 'DELETE', 'READ', 'DOWNLOAD', 'MODERATE', 'CREATE', 'CHANGE_PERMISSIONS'))
  new.acl = AccessControlList(id = id, resourceAccess = append(old.acl@resourceAccess, myaccess))
...
changeACL(COV_OBJ@properties$id)
...
changeACL(DM_OBJ@properties$id)
...
changeACL(FIT_OBJ@properties$id)
