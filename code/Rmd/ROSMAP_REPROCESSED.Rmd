...
METADATA <- METADATA %>%
  ...
  filter(!is.na(pmi)) %>%
  filter(!is.na(PCT_INTRONIC_BASES)) %>%
  as.data.frame()
...
1. Adding FLOWCELL and Gender a priori to variable selection
...
contrast = makeContrasts(contrasts=c("DiagnosisCONTROL-DiagnosisAD",
                                     "DiagnosisAD-DiagnosisOTHER",
                                     "DiagnosisCONTROL-DiagnosisOTHER"),
                         levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
...
rbindlist(all.diff.exp, use.names = T, fill = T) %>%
