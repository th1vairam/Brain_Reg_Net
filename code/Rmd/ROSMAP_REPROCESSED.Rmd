...
GENE.LEN = dplyr::select(GENE.PARAM, ensembl_gene_id, gene.length) %>% 
  unique() %>%
  left_join(data.frame(Gene.ID = rownames(COUNT)) %>%
              dplyr::mutate(tmp = Gene.ID) %>%
              tidyr::separate(tmp, c('ensembl_gene_id', 'position'), sep = '\\.'))
...
### Outlier Analysis
```{r outlier.analysis, cache = FALSE}
...
contrast = makeContrasts(contrasts=c("DiagnosisAD-DiagnosisCONTROL",
                                     "DiagnosisOTHER-DiagnosisAD",
                                     "DiagnosisOTHER-DiagnosisCONTROL"),
                         levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
...
SURR.VAR = sva::sva(CQN.GENE_EXPRESSION$y+CQN.GENE_EXPRESSION$offset, 
                    MODEL1, MODEL0, n.sv = NUM.SV, B = 3)$sv
