...
knit2synapse::knitToFolderEntity(file = "./CMC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_PITT.Rmd",
                                 parentId = "syn7501880",
                                 entityName = 'ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN - PITT')
```
...
thisFileName <- 'CMC_ACC_DLPFC_STAR_ENSEMBL_v75_CQN_PITT.Rmd'
...
METADATA = rbindlist(list(METADATA_ACC, METADATA_DLPFC), use.names = T, fill = T) %>%
  ...
  left_join(ANCESTRY) %>%
  dplyr::filter(Institution == 'Pitt', Dx %in% c('Control','SCZ')) %>%
  unique

# Replace AFF and BP samples as other
levels(METADATA$Dx) = c("Control", "SCZ")
```
...
COVARIATES.CORRELATION = getAssociationStatistics(COVARIATES[,setdiff(colnames(COVARIATES), 'Institution')], PVAL = 0.05)
...
# Code
CODE <- Folder(name = "ACC and DLPFC - STAR - FEATURE COUNTS - ENSEMBL v75 - CQN - PITT", parentId = parentId)
