---
title: "Covariate analysis of CMC RNASeq data reprocessed with Star-Feature counts pipeline (DLPFC with TMM and SVA normlaisation)"
...
metadataVarOfInterest = c("Individual ID", "Institution", "Gender", "Age of Death", "PMI (in hours)", 
                          "Dx", "DLPFC_RNA_isolation: Exclude?", "DLPFC_RNA_isolation: Sample RNA ID",
...
writeLines('Following samples are tagged as outliers')
METADATA <- METADATA  %>% filter(!(SampleID %in% c('MSSM_RNA_PFC_260', 'MSSM_RNA_PFC_60', 
                                                   'MSSM_RNA_PFC_139', 'MSSM_RNA_PFC_361', 
                                                   'MSSM_RNA_PFC_348', 'MSSM_RNA_PFC_389', 
                                                   'PENN_RNA_PFC_56', 'PENN_RNA_PFC_59'))) %>%
  as.data.frame()
```
...
FactorCovariates <- c("Institution", "Gender", "LibraryBatch", "Dx")
ContCovariates <- c("Age_of_Death", "PMI", "RIN")
...
### Normlaisation (Custom design)
To be consistent with DLPFC Nature Neuroscience article we performed a custom adjustment with Dx, RIN, RIN2, Age of Death, PMI, Library Batch, Institution, Gender
```{r custom.design}
# Add primary variable to the list (in case not chosen by iterative method)
postAdjustCovars = c('Dx', c(FactorCovariates, ContCovariates)) %>% unique()
...
# Fit contrast
contrast = makeContrasts(contrasts=c("DxControl-DxOther",
                                     "DxControl-DxSCZ",
                                     "DxOther-DxSCZ"),
