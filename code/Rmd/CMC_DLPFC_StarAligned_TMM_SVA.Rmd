---
title: "Covariate analysis of CMC RNASeq data reprocessed with Star-HTSeq pipeline (DLPFC with TMM and SVA normlaisation)"
...
knit2synapse::knitToFolderEntity(file = "./CMC_DLPFC_StarAligned_TMM_SVA.Rmd",
                                 parentId ="syn5569976",
                                 entityName = 'DLPFC - STAR Reprocessed - with TMM and SVA')
```
...
activityDescription = 'Covariate analysis of star aligned CMC reprocessed RNASeq data with TMM and SVA normalisation (DLPFC)';

thisFileName <- 'CMC_DLPFC_StarAligned_TMM_SVA.Rmd'
...
metadataVarOfInterest = c("Individual ID", "Institution", "Gender", "Ethnicity", "Age of Death", "PMI (in hours)", 
                  "pH", "Dx", "CDR", "CauseOfDeath", "DLPFC_RNA_isolation: Exclude?", "DLPFC_RNA_isolation: Sample RNA ID",
                  "DLPFC_RNA_isolation: RNA Isolation Batch", "DLPFC_RNA_isolation: RIN", "DLPFC_RNA_report: Exclude?",
                  "DLPFC_RNA_report: Library Batch")
...
METADATA = METADATA %>%
  dplyr::select(Individual_ID, Institution, Gender, Ethnicity, Age_of_Death, PMI__in_hours_, pH, Dx, CDR, CauseOfDeath,
                DLPFC_RNA_isolation__Exclude_, DLPFC_RNA_isolation__Sample_RNA_ID, DLPFC_RNA_isolation__RNA_Isolation_Batch,
                DLPFC_RNA_isolation__RIN, DLPFC_RNA_report__Exclude_, DLPFC_RNA_report__Library_Batch) %>%
  dplyr::rename(PMI = PMI__in_hours_, 
                IsolationExclude = DLPFC_RNA_isolation__Exclude_, 
                SampleID = DLPFC_RNA_isolation__Sample_RNA_ID, 
                IsolationBatch = DLPFC_RNA_isolation__RNA_Isolation_Batch,
...
METADATA <- METADATA %>%
  ...
  filter(!is.na(RIN)) %>%
  filter(!(SampleID %in% c('MSSM_RNA_PFC_260', 'MSSM_RNA_PFC_60', 'MSSM_RNA_PFC_139', 'MSSM_RNA_PFC_361', 
                           'MSSM_RNA_PFC_348', 'MSSM_RNA_PFC_389', 'PENN_RNA_PFC_56', 'PENN_RNA_PFC_59'))) %>%
  as.data.frame()
```
...
postAdjustCovars = c('Dx', 'RIN', 'RIN2', 'Age_of_Death', 'PMI', 'IsolationBatch')
...
```{r sva}
MODEL0 = DM1$design[,-grep('Dx',colnames(DM1$design))] # Get null model by removing variable of interest
MODEL1 = DM1$design

## Calculate the number of surrogate variables
NUM.SV = num.sv(VOOM.ADJUSTED.GENE_EXPRESSION$E, MODEL1, method = 'be', seed = 123456, B = 10)

SURR.VAR = sva(VOOM.ADJUSTED.GENE_EXPRESSION$E, MODEL1, MODEL0, method = "irw", n.sv = NUM.SV, B = 30)$sv
...
# Code
CODE <- Folder(name = "DLPFC - STAR Reprocessed - with TMM and SVA", parentId = parentId)
CODE <- synStore(CODE)

# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = 'CMC_DLPFC_Covariates.tsv', sep = '\t', row.names=F, quote=F)
COV_OBJ = File('CMC_DLPFC_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
COV_OBJ = synStore(COV_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                   executed = thisFile, activityDescription = activityDescription)

# Store logCPM
write.table(VOOM.ADJUSTED.GENE_EXPRESSION$E, file = 'CMC_DLPFC_logCPM.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_DLPFC_logCPM.tsv', name = 'Raw counts (logCPM)', parentId = CODE$properties$id)
COUNT_OBJ = synStore(COUNT_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                     executed = thisFile, activityDescription = activityDescription)

# Store design matrix
write.table(DM1$design, file = 'CMC_DLPFC_Design.tsv', sep = '\t', row.names=F, quote=F)
DM_OBJ = File('CMC_DLPFC_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
DM_OBJ = synStore(DM_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                  executed = thisFile, activityDescription = activityDescription)

# Store residual gene expression
write.table(RESIDUAL.GENE_EXPRESSION, file = 'CMC_DLPFC_Expression.tsv', sep = '\t', row.names=F, quote=F)
EXP_OBJ = File('CMC_DLPFC_Expression.tsv', name = 'Normalised Adjusted Residual Expression (Dx added)', parentId = CODE$properties$id)
EXP_OBJ = synStore(EXP_OBJ, used = ALL_USED_IDs, activityName = activityName, 
                   executed = thisFile, activityDescription = activityDescription)

# Store differential expression results
write.table(DE, file = 'CMC_DLPFC_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_DLPFC_DiffExpression.tsv', name = 'Differential Expression Analysis', parentId = CODE$properties$id)
