...
covariatesAdjustments <- function(exprId, regionName, covId){
  inline_expr(paste0('Working on ',regionName), "md")
  ...
  inline_expr(paste('Significant covariates are:',paste(preAdjustedSigCovars$significantCovars,collapse=',')), "md")

  # Get design matrix
  DESIGN <- getDesignMatrix(covar[,'Dx',drop=F], Intercept = F)
  inline_expr(paste('Fitting linear model with the following coefficients:',paste(colnames(DESIGN$design),collapse=',')), "md")
  ...
  if (length(postAdjustedSigCovars$significantCovars) != 0)
    inline_expr(paste('Following coefficients have to be included in the model:',paste(postAdjustedSigCovars$significantCovars,collapse=',')), "md")
...
```{r covariate.analysis, cache=TRUE, fig.height=10, fig.width=15}
...
adjustedExpression <- mapply(covariatesAdjustments, EXP_ID, names(EXP_ID), MoreArgs= list(COV_ID), SIMPLIFY=T)
```
