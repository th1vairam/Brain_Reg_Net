...
```{r detect.outlier.genes, cache = FALSE}
...
plotdata$label[!(plotdata$label %in% top.nsamples)] = ''
...
cr = cor(t(RESIDUAL.GENE_EXPRESSION %>% changeNAtoZero))
hist(cr, main = 'Distribution of correlation between genes', xlab = 'Correlation')
```
...
# Find principal components of expression to plot
PC <- prcomp(RESIDUAL.GENE_EXPRESSION %>% changeNAtoZero, scale.=T, center = T)
...
tree = hclust(as.dist(t(RESIDUAL.GENE_EXPRESSION %>% changeNAtoZero)))
cols = WGCNA::labels2colors(COVARIATES.tmp);
...
names(DE)[5:7] = c('SCZ_ALL-Control_ALL', 'ALL_DLPFC-ALL_ACC', 'ALL_ALL-ALL_ALL')
...
```{r diffExp1, fig.height=10, fig.width=15}
postAdjustCovars = c('Tissue', 'Dx', 'Institution', 'Gender', 'RIN', 'IntronicRate')

# Post adjusted design matrix
COV1 = COVARIATES
COV1 = tidyr::separate(COV1, Dx.Tissue, c('Dx', 'Tissue'), sep = '_') %>%
  dplyr::mutate(Dx = factor(Dx, levels = c('SCZ', 'Control', 'Other')), Tissue = factor(Tissue))

DESIGN = getDesignMatrix(COV1[,postAdjustCovars,drop=F],Intercept = F)$design
DESIGN = DESIGN[,linColumnFinder(DESIGN)$indepCols]
  
# Re-calculate voom weights with correlation of random effects
VOOM.GENE_EXPRESSION = voom(PROCESSED_COUNTS$filteredExprMatrix$counts %>% changeNAtoZero,   
                            design = DESIGN, plot=F,
                            block = COV1$Individual_ID, 
                            correlation = correlation$cor)
VOOM.GENE_EXPRESSION$E = CQN.GENE_EXPRESSION$E
...
names(DE1) = c('ACC-DLPFC', 'SCZ-CONTROL')
...
diffexp.genes.up$ACC.DLPFC.UP = dplyr::filter(DE1, adj.P.Val <= 0.05, logFC > 0, Comparison == 'ACC-DLPFC') %>%
  dplyr::select(hgnc_symbol) %>%
  unique() %>% unlist() %>% as.character()
diffexp.genes.down$ACC.DLPFC.DOWN = dplyr::filter(DE1, adj.P.Val <= 0.05, logFC < 0, Comparison == 'ACC-DLPFC') %>%
  dplyr::select(hgnc_symbol) %>%
  unique() %>% unlist() %>% as.character()
diffexp.genes.up$ACC.DLPFC.UNCHANGED = dplyr::filter(DE1, adj.P.Val > 0.05, Comparison == 'ACC-DLPFC') %>%
  dplyr::select(hgnc_symbol) %>%
  unique() %>% unlist() %>% as.character()

diffexp.genes = c(diffexp.genes.up, diffexp.genes.down)
diffexp.genes = diffexp.genes[c("Control.ACC.SCZ.ACC.UP", "Control.ACC.SCZ.ACC.DOWN",
                                "Control.DLPFC.SCZ.DLPFC.UP", "Control.DLPFC.SCZ.DLPFC.DOWN",
                                "ACC.DLPFC.UP", "ACC.DLPFC.DOWN", "ACC.DLPFC.UNCHANGED")]
diffexp.genes$Control.SCZ.ACC.DLPFC.UP = setdiff(diffexp.genes$Control.ACC.SCZ.ACC.UP, diffexp.genes$Control.DLPFC.SCZ.DLPFC.UP)
diffexp.genes$Control.SCZ.ACC.DLPFC.DOWN = setdiff(diffexp.genes$Control.ACC.SCZ.ACC.DOWN, diffexp.genes$Control.DLPFC.SCZ.DLPFC.DOWN)
diffexp.genes$Control.SCZ.DLPFC.ACC.UP = setdiff(diffexp.genes$Control.DLPFC.SCZ.DLPFC.UP, diffexp.genes$Control.ACC.SCZ.ACC.UP)
diffexp.genes$Control.SCZ.DLPFC.ACC.DOWN = setdiff(diffexp.genes$Control.DLPFC.SCZ.DLPFC.DOWN, diffexp.genes$Control.ACC.SCZ.ACC.DOWN)
diffexp.genes$Control.SCZ.DLPFC.ACC.BOTH.UP = intersect(diffexp.genes$Control.DLPFC.SCZ.DLPFC.UP, diffexp.genes$Control.ACC.SCZ.ACC.UP)
diffexp.genes$Control.SCZ.DLPFC.ACC.BOTH.DOWN = intersect(diffexp.genes$Control.DLPFC.SCZ.DLPFC.DOWN, diffexp.genes$Control.ACC.SCZ.ACC.DOWN)
